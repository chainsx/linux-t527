...
{
	return system_heap_do_allocate(heap, len, (u32)fd_flags, (u64)heap_flags, true);
}
...
struct device *find_device_globally(const char *device_name)
{
	struct device *dev;
	struct kobject *kobj;
	...
	list_for_each_entry(kobj, &devices_kset->list, entry) {
		...
		if (dev->kobj.name && strcmp(dev->kobj.name, device_name) == 0) {
			get_device(dev);
			spin_unlock(&devices_kset->list_lock);
			return dev;
		}

		if (dev->init_name && strcmp(dev->init_name, device_name) == 0) {
			get_device(dev);
			spin_unlock(&devices_kset->list_lock);
			return dev;
		}
...
static int system_uncached_heap_create(void)
{
	...
	heap_dev = find_device_globally("system-uncached");
	if (!heap_dev) {
		pr_err("class_find_device_by_devt failed\n");
		return -1;
	}
...
module_init(system_uncached_heap_create);
