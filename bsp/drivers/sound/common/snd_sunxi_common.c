...
static int pacfg_level_trig_init(struct snd_sunxi_pacfg *pa_cfg)
{
	...
	ret = devm_gpio_request_one(&pdev->dev, gpio_tmp, GPIOF_OUT_INIT_HIGH, str);
	if (ret) {
...
{
	struct snd_sunxi_pacfg *pa_cfg = container_of(work, struct snd_sunxi_pacfg,
						      pa_en_work.work);
	struct pacfg_level_trig *level_trig = pa_cfg->level_trig;
	unsigned long flags;
...
static void pacfg_level_trig_disable(struct snd_sunxi_pacfg *pa_cfg)
{
	...
	spin_lock_irqsave(&pa_enable_lock, flags);
	if (pa_cfg->status != SND_SUNXI_PACFG_STA_OPEN) {
		pa_cfg->status = SND_SUNXI_PACFG_STA_STOP;
		spin_unlock_irqrestore(&pa_enable_lock, flags);
		cancel_delayed_work_sync(&pa_cfg->pa_en_work);
		spin_lock_irqsave(&pa_enable_lock, flags);
		pa_cfg->status = SND_SUNXI_PACFG_STA_CLOSE;
		spin_unlock_irqrestore(&pa_enable_lock, flags);
		return;
	}
	...
	*pa_pin_max = 0;
	...
	for (i = 0; i < pin_max; i++) {
		...
		case SND_SUNXI_PA_CFG_LEVEL:
			...
			INIT_DELAYED_WORK(&pa_cfg[i].pa_en_work, pacfg_level_trig_enable);
			INIT_DELAYED_WORK(&pa_cfg[i].pa_disa_work, pacfg_level_trig_disa_work);
			pa_cfg[i].used = true;
			break;
		case SND_SUNXI_PA_CFG_PULSE:
			...
			INIT_DELAYED_WORK(&pa_cfg[i].pa_en_work, pacfg_pulse_trig_enable);
			INIT_DELAYED_WORK(&pa_cfg[i].pa_disa_work, pacfg_pulse_trig_disa_work);
			pa_cfg[i].used = true;
			break;
...
void snd_sunxi_pa_pin_exit(struct snd_sunxi_pacfg *pa_cfg, u32 pa_pin_max)
{
	...
	for (i = 0; i < pa_pin_max; i++) {
		...
		case SND_SUNXI_PA_CFG_LEVEL:
			...
			cancel_delayed_work_sync(&pa_cfg[i].pa_en_work);
			cancel_delayed_work_sync(&pa_cfg[i].pa_disa_work);
			break;
		case SND_SUNXI_PA_CFG_PULSE:
...
int snd_sunxi_pa_pin_enable(struct snd_sunxi_pacfg *pa_cfg, u32 pa_pin_max)
{
	...
	for (i = 0; i < pa_pin_max; i++) {
		...
		switch (pa_cfg[i].mode) {
		case SND_SUNXI_PA_CFG_LEVEL:
			schedule_delayed_work(&pa_cfg[i].pa_en_work,
					      msecs_to_jiffies(pa_cfg[i].level_trig->msleep_0));
			break;
		case SND_SUNXI_PA_CFG_PULSE:
			schedule_delayed_work(&pa_cfg[i].pa_en_work,
					      msecs_to_jiffies(pa_cfg[i].pulse_trig->msleep_0));
			break;
		case SND_SUNXI_PA_CFG_USER:
				  ...
				  void (*pa_disa_cb)(void *data))
{
	...
	for (i = 0; i < pa_pin_max; i++) {
		...
		case SND_SUNXI_PA_CFG_LEVEL:
			...
			if (pa_disa_cb)
				schedule_delayed_work(&pa_cfg[i].pa_disa_work,
						      msecs_to_jiffies(pa_cfg[i].
						      level_trig->msleep_1));
			break;
		case SND_SUNXI_PA_CFG_PULSE:
			pacfg_pulse_trig_disable(&pa_cfg[i]);
...
static int __init snd_sunxi_dev_init(void)
{
	SND_LOG_DEBUG("\n");
	return _snd_sunxi_dev_init(&sunxi_dev);
}

static void __exit snd_sunxi_dev_exit(void)
{
	SND_LOG_DEBUG("\n");
