...
static int sunxi_ahub_dai_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	ret = snd_ahub_i2s_clk_enable(ahub->i2s_clk);
	if (ret < 0) {
		SND_LOG_ERR("ahub i2s clk enable failed\n");
		return ret;
	}
...
{
	struct sunxi_ahub *ahub = snd_soc_dai_get_drvdata(dai);
	struct sunxi_ahub_i2s_clk_sta *ahub_i2s_clk_sta = &ahub->ahub_i2s_clk_sta;
	struct regmap *regmap = NULL;
	...
	mutex_lock(&ahub->ahub_i2s_clk_sta.ahub_clk_mutex);
	if (!ahub->ahub_i2s_clk_sta.p_work && !ahub->ahub_i2s_clk_sta.c_work) {
		if (ahub->i2s_clk) {
			if (snd_ahub_i2s_clk_rate(ahub->i2s_clk,
						  ahub->pllclk_freq,
						  ahub->mclk_freq)) {
				SND_LOG_ERR("i2s clk set rate failed\n");
				return -EINVAL;
			}
		}
	}
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ahub_i2s_clk_sta->p_work = 1;
	} else {
		ahub_i2s_clk_sta->c_work = 1;
	}
	mutex_unlock(&ahub->ahub_i2s_clk_sta.ahub_clk_mutex);
...
static int sunxi_ahub_dai_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	mutex_lock(&ahub->ahub_i2s_clk_sta.ahub_clk_mutex);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (!ahub_i2s_clk_sta->p_work)
			goto exit;

		if (!ahub_i2s_clk_sta->c_work) {
			snd_ahub_i2s_clk_disable(ahub->i2s_clk);
		}
		ahub_i2s_clk_sta->p_work = 0;
	} else {
		if (!ahub_i2s_clk_sta->c_work)
			goto exit;
...
static int sunxi_ahub_probe(struct snd_soc_component *component)
{
	...
	mutex_init(&ahub->ahub_i2s_clk_sta.ahub_clk_mutex);
...
static void sunxi_ahub_remove(struct snd_soc_component *component)
{
	...
	mutex_destroy(&ahub->ahub_i2s_clk_sta.ahub_clk_mutex);
...
static int sunxi_ahub_dev_probe(struct platform_device *pdev)
{
	...
	ahub->dev = &pdev->dev;
	mem = &ahub->mem;
	clk = (struct sunxi_ahub_clk *)ahub->ahub_clk;
	pin = &ahub->pin;
	dts = &ahub->dts;
	rglt = &ahub->rglt;
	...
	ahub->ahub_clk = snd_sunxi_ahub_clk_get();
	if (IS_ERR_OR_NULL(ahub->ahub_clk)) {
		SND_LOG_ERR("clk get failed\n");
		ret = -EPROBE_DEFER;
		goto err_ahub_clk_get;
	}

	ahub->i2s_clk = snd_ahub_i2s_clk_init(pdev);
	if (!ahub->i2s_clk) {
		SND_LOG_ERR("ahub i2s clk init failed\n");
		ret = -EINVAL;
		goto err_snd_ahub_i2s_clk_init;
	}
...
err_snd_ahub_i2s_clk_init:
	snd_sunxi_ahub_clk_exit(ahub->ahub_clk);
err_ahub_clk_get:
...
#endif
{
	...
	snd_ahub_i2s_clk_exit(ahub->i2s_clk);
...
int __init sunxi_ahub_dev_init(void)
{
	...
	platform_driver = sunxi_get_ahub_dam_drv();
	ret = platform_driver_register(platform_driver);
	if (ret != 0) {
		SND_LOG_ERR("dam platform driver register failed\n");
	...
	if (ret != 0) {
		SND_LOG_ERR("ahub platform driver register failed\n");
...
{
	struct platform_driver *platform_driver = sunxi_get_ahub_dam_drv();
