				     ...
				     struct snd_soc_dai *dai)
{
	...
	/* Enable DAC Mono register to send 1ch data */
...
/* Digital-tlv */
static const DECLARE_TLV_DB_SCALE(dac_vol_tlv, 1800, -56, 0);
static const DECLARE_TLV_DB_SCALE(dacl_vol_tlv, -6400, 50, 1);
static const DECLARE_TLV_DB_SCALE(adc1_vol_tlv, -6400, 50, 1);
static const DECLARE_TLV_DB_SCALE(adc2_vol_tlv, -6400, 50, 1);
/* Analog-tlv */
static const unsigned int adc1_gain_tlv[] = {
	TLV_DB_RANGE_HEAD(4),
	0, 0, TLV_DB_SCALE_ITEM(0, 0, 0),
	1, 3, TLV_DB_SCALE_ITEM(600, 0, 0),
	4, 12, TLV_DB_SCALE_ITEM(600, 300, 0),
	13, 15, TLV_DB_SCALE_ITEM(3600, 0, 0),
};
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	...
	SOC_SINGLE_RANGE_TLV("ADC1 Gain", SUNXI_ADC1_REG, ADC1_PGA_GAIN_CTRL, 0x0, 0xF, 0, adc1_gain_tlv),
	SOC_SINGLE_RANGE_TLV("ADC2 Gain", SUNXI_ADC2_REG, ADC2_PGA_GAIN_CTRL, 0x0, 0xF, 0, adc2_gain_tlv),
};
...
static const struct snd_soc_dapm_widget sunxi_codec_dapm_widgets[] = {
	...
	SND_SOC_DAPM_AIF_OUT("ADC1", "Capture", 0, SUNXI_ADC_DIG_CTRL, 0, 0),
	SND_SOC_DAPM_AIF_OUT("ADC2", "Capture", 0, SUNXI_ADC_DIG_CTRL, 1, 0),
...
static void sunxi_codec_init(struct snd_soc_component *component)
{
	...
	regmap_update_bits(regmap, SUNXI_DAC_DHP_REG, 0x1 << DAC_DHP_GAIN_BYPASS, 0x1 << DAC_DHP_GAIN_BYPASS);
...
static int snd_sunxi_clk_init(struct platform_device *pdev, struct sunxi_codec_clk *clk)
{
	...
	if (IS_ERR_OR_NULL(clk->clk_pll_audio_1x)) {
		SND_LOG_ERR_STD(E_AUDIOCODEC_SWDEP_CLK_INIT, "clk_pll_audio_1x get failed\n");
...
static int snd_sunxi_clk_enable(struct sunxi_codec_clk *clk)
{
	...
	if (clk_prepare_enable(clk->clk_pll_audio_1x)) {
		SND_LOG_ERR_STD(E_AUDIOCODEC_SWDEP_CLK_EN, "clk_pll_audio_1x enable failed\n");
...

{
	...
	ret = of_property_read_u32(np, "lineoutl-vol", &temp_val);
	if (ret < 0) {
