				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	regmap_read(dig_regmap, SUNXI_ADDA1_SYNC_CTL, &reg_val);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (ucontrol->value.integer.value[0]) {
	case 0:
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL, 0x1 << HUB_EN, 0x0 << HUB_EN);
		break;
	case 1:
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL, 0x1 << HUB_EN, 0x1 << HUB_EN);
		break;
	default:
		return -EINVAL;
	}
...
static const struct snd_soc_dapm_widget ac203c_dapm_widgets[] = {
	...
	SND_SOC_DAPM_LINE("LINEOUTL", ac203c_lineoutl_event),
	SND_SOC_DAPM_LINE("LINEOUTR", ac203c_lineoutr_event),
