...
#define SUNXI_MODNAME		"sound-ahub-dmic"
				    ...
				    struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_DMIC_TX, 16);
		snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_DMIC_TX, 1);
		regmap_update_bits(regmap, SUNXI_DMIC_FIFO_CTL,
	...
	case SNDRV_PCM_FORMAT_S24_LE:
		snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_DMIC_TX, 32);
		snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_DMIC_TX, 0);
		regmap_update_bits(regmap, SUNXI_DMIC_FIFO_CTL,
				   0x1 << DMIC_SAMPLE_RESOLUTION,
				   0x1 << DMIC_SAMPLE_RESOLUTION);
		regmap_update_bits(regmap, SUNXI_DMIC_FIFO_CTL,
	...
	if (params_rate(params) >= 24000)
		regmap_update_bits(regmap, SUNXI_DMIC_CTL,
	...
	/* set channels */
	snd_sunxi_set_apbif_ch(SUNXI_AHUB_PORT_DMIC_TX, params_channels(params));
	channels = snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_DMIC_TX, params_channels(params), true);
	if (!channels)
		channels = params_channels(params);
	snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_DMIC_TX, true);
	...
	for (i = 0; i < channels; i++) {
		...
		/* enabled HPF */
		regmap_update_bits(regmap, SUNXI_DMIC_HPF_CTL,
				   0x1 << (HPF_DATA0_CHL_EN + channels_en),
				   0x1 << (HPF_DATA0_CHL_EN + channels_en));
	}

	snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_DMIC_TX, true);
...
static int sunxi_dmic_dai_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_DMIC_TX, false);
	...
	snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_DMIC_TX, 0, false);
	snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_DMIC_TX, false);
...
static int sunxi_dmic_dai_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	snd_sunxi_flush_fifo(SUNXI_AHUB_PORT_DMIC_TX);

	regmap_write(regmap, SUNXI_DMIC_INTS,
		     (1 << FIFO_OVERRUN_IRQ_PENDING) | (1 << FIFO_UNDERRUN_IRQ_PENDING));
...
static void sunxi_dmic_dai_rx_route(struct sunxi_dmic *dmic, bool enable)
{
	...
	if (enable) {
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_DMIC_TX, true);
		...
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_DMIC_TX, false);
	}
}
...
static int sunxi_dmic_init(struct sunxi_dmic *dmic)
{
	...
	if (dts->rx_dtime) {
		regmap_update_bits(regmap, SUNXI_DMIC_CTL, 0x3 << DMICFDT, rx_dtime_map << DMICFDT);
		regmap_update_bits(regmap, SUNXI_DMIC_CTL, 0x1 << DMICDFEN, 0x1 << DMICDFEN);
	} else {
		regmap_update_bits(regmap, SUNXI_DMIC_CTL, 0x1 << DMICDFEN, 0x0 << DMICDFEN);
	}
	...
	regmap_update_bits(regmap, SUNXI_DMIC_CTL, 1 << DATA0_LR_SWAP_EN, 0 << DATA0_LR_SWAP_EN);
	regmap_update_bits(regmap, SUNXI_DMIC_CTL, 1 << DATA1_LR_SWAP_EN, 0 << DATA1_LR_SWAP_EN);
	regmap_update_bits(regmap, SUNXI_DMIC_CTL, 1 << DATA2_LR_SWAP_EN, 0 << DATA2_LR_SWAP_EN);
	regmap_update_bits(regmap, SUNXI_DMIC_CTL, 1 << DATA3_LR_SWAP_EN, 0 << DATA3_LR_SWAP_EN);
...
static const struct snd_kcontrol_new sunxi_rx_sync_controls[] = {
	SOC_ENUM_EXT("dmic rx sync mode", sunxi_rx_sync_mode_enum,
...
static const struct snd_kcontrol_new sunxi_dmic_controls[] = {
	/* Digital Volume */
	SOC_SINGLE_TLV("dmic L0 volume", SUNXI_DMIC_DATA0_1_VOL, DATA0L_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic R0 volume", SUNXI_DMIC_DATA0_1_VOL, DATA0R_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic L1 volume", SUNXI_DMIC_DATA0_1_VOL, DATA1L_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic R1 volume", SUNXI_DMIC_DATA0_1_VOL, DATA1R_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic L2 volume", SUNXI_DMIC_DATA2_3_VOL, DATA2L_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic R2 volume", SUNXI_DMIC_DATA2_3_VOL, DATA2R_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic L3 volume", SUNXI_DMIC_DATA2_3_VOL, DATA3L_VOL, 0xFF, 0, digital_tlv),
	SOC_SINGLE_TLV("dmic R3 volume", SUNXI_DMIC_DATA2_3_VOL, DATA3R_VOL, 0xFF, 0, digital_tlv),
};
...
static void snd_sunxi_dma_params_init(struct sunxi_dmic *dmic)
{
	...
	dmic->capture_dma_param.dma_addr = snd_sunxi_get_dev_fifo(SUNXI_AHUB_PORT_DMIC_TX);
...
static int sunxi_dmic_dev_probe(struct platform_device *pdev)
{
	...
	/* soft reset */
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_DMIC_TX);
...
MODULE_DESCRIPTION("sunxi soundcard platform of ahub dmic");
