...
#define SUNXI_MODNAME		"sound-ahub-ac203c"
...
#define DRV_NAME	"sunxi-snd-codec-ahub-ac203c"
			  ...
			  int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (ac203c->pdata.ac203c_sta.spk)
			snd_sunxi_pa_pin_enable(ac203c->pa_cfg, ac203c->pa_pin_max);
		fallthrough;
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_CODEC_RX, true);
		} else {
			snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_CODEC_TX, true);
			if (ac203c->rx_sync_ctl)
				sunxi_rx_sync_control(ac203c->rx_sync_domain,
						      ac203c->rx_sync_id, true);
		}
		break;
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (ac203c->pdata.ac203c_sta.spk)
			snd_sunxi_pa_pin_disable_irp(ac203c->pa_cfg, ac203c->pa_pin_max,
						     NULL, NULL);
		fallthrough;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_CODEC_RX, false);
		} else {
			snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_CODEC_TX, false);
			if (ac203c->rx_sync_ctl)
				sunxi_rx_sync_control(ac203c->rx_sync_domain,
						      ac203c->rx_sync_id, false);
		}
		break;
	default:
		return -EINVAL;
			   ...
			   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_CODEC_RX, 16);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_CODEC_RX, 1);
			regmap_update_bits(dig_regmap, SUNXI_DAC_DIG_CTL,
					   0x1 << DAC_FIFO_MODE, 0x1 << DAC_FIFO_MODE);
			regmap_update_bits(dig_regmap, SUNXI_DAC_DIG_CTL,
					   0x3 << DAC_SAMPLE_WIDTH, 0x2 << DAC_SAMPLE_WIDTH);
		} else {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_CODEC_TX, 16);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_CODEC_TX, 1);
			regmap_update_bits(dig_regmap, SUNXI_ADC_DDT_CTL,
					   0x1 << ADC_FIFO_MODE, 0x1 << ADC_FIFO_MODE);
			regmap_update_bits(dig_regmap, SUNXI_ADC_DDT_CTL,
					   0x3 << ADC_SAMPLE_WIDTH, 0x2 << ADC_SAMPLE_WIDTH);
		}
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
	case SNDRV_PCM_FORMAT_S32_LE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_CODEC_RX, 32);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_CODEC_RX, 0);
			regmap_update_bits(dig_regmap, SUNXI_DAC_DIG_CTL,
					   0x3 << DAC_FIFO_MODE, 0x1 << DAC_FIFO_MODE);
			regmap_update_bits(dig_regmap, SUNXI_DAC_DIG_CTL,
					   0x1 << DAC_SAMPLE_WIDTH, 0x0 << DAC_SAMPLE_WIDTH);
		} else {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_CODEC_TX, 32);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_CODEC_TX, 0);
			regmap_update_bits(dig_regmap, SUNXI_ADC_DDT_CTL,
					   0x1 << ADC_FIFO_MODE, 0x1 << ADC_FIFO_MODE);
			regmap_update_bits(dig_regmap, SUNXI_ADC_DDT_CTL,
					   0x3 << ADC_SAMPLE_WIDTH, 0x0 << ADC_SAMPLE_WIDTH);
		}
		break;
	default:
		break;
	...
	if (snd_ac203c_clk_enable(ac203c->clk)) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_CODEC_RX, true);
		snd_sunxi_set_apbif_ch(SUNXI_AHUB_PORT_CODEC_RX, params_channels(params));
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_CODEC_RX, params_channels(params), true);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_CODEC_RX, true);
		...
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_CODEC_TX, true);
		snd_sunxi_set_apbif_ch(SUNXI_AHUB_PORT_CODEC_TX, params_channels(params));
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_CODEC_TX, params_channels(params), true);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_CODEC_TX, true);
...
static int ac203c_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_CODEC_RX, false);
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_CODEC_RX, 0, false);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_CODEC_RX, false);
	} else {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_CODEC_TX, false);
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_CODEC_TX, 0, false);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_CODEC_TX, false);
	}
	...
	if (ac203c->clk_sta == SND_SUNXI_CLK_OPEN) {
		snd_ac203c_clk_disable(ac203c->clk);
...
static int ac203c_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_flush_fifo(SUNXI_AHUB_PORT_CODEC_RX);
		sunxi_regmap_write(dig_regmap, SUNXI_ADDA1_DAC_CNT, 0x0);
	} else {
		snd_sunxi_flush_fifo(SUNXI_AHUB_PORT_CODEC_TX);
			  ...
			  unsigned int freq_in, unsigned int freq_out)
{
	...
	return snd_ac203c_clk_rate(ac203c->clk, freq_in, freq_out);
}
...
{
	struct regmap *dig_regmap = data;

	SND_LOG_DEBUG("%s\n", enable ? "on" : "off");

	if (enable) {
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL,
				   0x1 << SYNC_EN, 0x1 << SYNC_EN);
	} else {
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL,
				   0x1 << SYNC_EN, 0x0 << SYNC_EN);
	}
...
static int ac203c_probe(struct snd_soc_component *component)
{
	...
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_CODEC_RX);
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_CODEC_TX);

	mutex_init(&pdata->ac203c_sta.audio_mutex);

	/* component -> rx_sync */
	ac203c->rx_sync_ctl = false;
	ac203c->rx_sync_domain = RX_SYNC_SYS_DOMAIN;
	ac203c->rx_sync_id = sunxi_rx_sync_probe(ac203c->rx_sync_domain);
	if (ac203c->rx_sync_id < 0) {
		SND_LOG_ERR("sunxi_rx_sync_probe failed\n");
	} else {
		SND_LOG_DEBUG("sunxi_rx_sync_probe successful. domain=%d, id=%d\n",
			      ac203c->rx_sync_domain, ac203c->rx_sync_id);
		ret = sunxi_rx_sync_register_cb(ac203c->rx_sync_domain, ac203c->rx_sync_id,
						(void *)dig_regmap, sunxi_rx_sync_enable);
		if (ret)
	...
	sunxi_regmap_update_bits(ana_regmap, SUNXI_HP_REG1,
				 0x1 << HPOUT_SHORT_CTL, 0x1 << HPOUT_SHORT_CTL);
	...
	sunxi_regmap_update_bits(ana_regmap, SUNXI_POWER_REG5,
				 0xE << HP_USE, 0xE << HP_USE);
	...
	/* en */
	sunxi_regmap_update_bits(ana_regmap, SUNXI_I2S_CTL_2, 0x1 << I2S_TX_EN, 0x1 << I2S_TX_EN);
	sunxi_regmap_update_bits(ana_regmap, SUNXI_I2S_CTL_2, 0x1 << I2S_RX_EN, 0x1 << I2S_RX_EN);
...
static void ac203c_remove(struct snd_soc_component *component)
{
	...
	sunxi_rx_sync_unregister_cb(ac203c->rx_sync_domain, ac203c->rx_sync_id);
	sunxi_rx_sync_remove(ac203c->rx_sync_domain);
...
static int ac203c_suspend(struct snd_soc_component *component)
{
	...
	snd_ac203c_clk_bus_disable(ac203c->clk);
...
static int ac203c_resume(struct snd_soc_component *component)
{
	...
	ret = snd_ac203c_clk_bus_enable(ac203c->clk);
	if (ret)
		return ret;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = ac203c->rx_sync_ctl;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ac203c->rx_sync_ctl == ucontrol->value.integer.value[0])
		return 0;

	switch (ucontrol->value.integer.value[0]) {
	case 0:
		ac203c->rx_sync_ctl = 0;
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL, 1 << EN_MUX, 0 << EN_MUX);
		sunxi_rx_sync_shutdown(ac203c->rx_sync_domain, ac203c->rx_sync_id);
		break;
	case 1:
		regmap_update_bits(dig_regmap, SUNXI_ADDA1_SYNC_CTL, 1 << EN_MUX, 1 << EN_MUX);
		ac203c->rx_sync_ctl = 1;
		sunxi_rx_sync_startup(ac203c->rx_sync_domain, ac203c->rx_sync_id);
		break;
	default:
		return -EINVAL;
	}
...
static int ac203c_spk_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		ac203c->pdata.ac203c_sta.spk = true;
		snd_sunxi_pa_pin_enable(ac203c->pa_cfg, ac203c->pa_pin_max);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		ac203c->pdata.ac203c_sta.spk = false;
		snd_sunxi_pa_pin_disable(ac203c->pa_cfg, ac203c->pa_pin_max);
		break;
	default:
		break;
	}
...
static const struct snd_kcontrol_new adc1_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c ADC1 Data Select Mux", adc1_src_mux_enum,
			  ac203c_get_adc1_src,
			  ac203c_set_adc1_src);
static const struct snd_kcontrol_new adc2_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c ADC2 Data Select Mux", adc2_src_mux_enum,
			  ac203c_get_adc2_src,
			  ac203c_set_adc2_src);
static const struct snd_kcontrol_new adc3_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c ADC3 Data Select Mux", adc3_src_mux_enum,
			  ac203c_get_adc3_src,
			  ac203c_set_adc3_src);

static const struct snd_kcontrol_new adc2_input_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c ADC2 Input Src Mux", adc2_input_mux_enum,
			  ac203c_get_adc2_input_src,
			  ac203c_set_adc2_input_src);
static const struct snd_kcontrol_new adc3_input_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c ADC3 Input Src Mux", adc3_input_mux_enum,
			  ac203c_get_adc3_input_src,
			  ac203c_set_adc3_input_src);

static const struct snd_kcontrol_new rxm1_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c RX1 Mux", rxm1_src_mux_enum,
			  ac203c_get_rxm1_src,
			  ac203c_set_rxm1_src);
static const struct snd_kcontrol_new rxm2_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c RX2 Mux", rxm2_src_mux_enum,
			  ac203c_get_rxm2_src,
			  ac203c_set_rxm2_src);
static const struct snd_kcontrol_new rxm3_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c RX3 Mux", rxm3_src_mux_enum,
			  ac203c_get_rxm3_src,
			  ac203c_set_rxm3_src);

static const struct snd_kcontrol_new txm1_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c AIF TX1 Mux", txm1_src_mux_enum,
			  ac203c_get_txm1_src,
			  ac203c_set_txm1_src);
static const struct snd_kcontrol_new txm2_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c AIF TX2 Mux", txm2_src_mux_enum,
			  ac203c_get_txm2_src,
			  ac203c_set_txm2_src);
static const struct snd_kcontrol_new txm3_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c AIF TX3 Mux", txm3_src_mux_enum,
			  ac203c_get_txm3_src,
			  ac203c_set_txm3_src);

static const struct snd_kcontrol_new dacl_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c DACL Data Select Mux", dac1_src_mux_enum,
			  ac203c_get_dacl_src,
			  ac203c_set_dacl_src);
static const struct snd_kcontrol_new dacr_src_mux =
	SOC_DAPM_ENUM_EXT("ac203c DACR Data Select Mux", dac2_src_mux_enum,
			  ac203c_get_dacr_src,
			  ac203c_set_dacr_src);

static const struct snd_kcontrol_new ac203c_snd_controls[] = {
	/* rx sync */
	SOC_ENUM_EXT("ac203c rx sync mode", sunxi_rx_sync_mode_enum,
		     sunxi_get_rx_sync_mode, sunxi_set_rx_sync_mode),

	/* adc dig vol*/
	SOC_SINGLE_TLV("ac203c ADC1 Volume", SUNXI_ADC_DVOL_CTL, DIG_ADC1_VOL, 0xff, 0, adc_dig_vol_tlv),
	SOC_SINGLE_TLV("ac203c ADC2 Volume", SUNXI_ADC_DVOL_CTL, DIG_ADC2_VOL, 0xff, 0, adc_dig_vol_tlv),
	SOC_SINGLE_TLV("ac203c ADC3 Volume", SUNXI_ADC_DVOL_CTL, DIG_ADC3_VOL, 0xff, 0, adc_dig_vol_tlv),
	/* dac dig vol */
	SOC_SINGLE_TLV("ac203c DACL Volume", SUNXI_DAC_DVC, DAC_DVC_L, 0xff, 0, dac_dig_vol_tlv),
	SOC_SINGLE_TLV("ac203c DACR Volume", SUNXI_DAC_DVC, DAC_DVC_R, 0xff, 0, dac_dig_vol_tlv),
	/* dac gain */
	SOC_SINGLE_TLV("ac203c DAC Gain", SUNXI_DAC_DHP_GAIN_CTL, DHP_OUT_GAIN, 0x7, 1, dac_gain_tlv),
	/* adc gain */
	SOC_SINGLE_TLV("ac203c ADC1 Gain", SUNXI_ADC1_REG3, ADC1_PGA_GAIN_CTL, 0xf, 0, adc_gain_tlv),
	SOC_SINGLE_TLV("ac203c ADC2 Gain", SUNXI_ADC2_REG3, ADC2_PGA_GAIN_CTL, 0xf, 0, adc_gain_tlv),
	SOC_SINGLE_TLV("ac203c ADC3 Gain", SUNXI_ADC3_REG3, ADC3_PGA_GAIN_CTL, 0xf, 0, adc_gain_tlv),

	/* swap */
	SOC_ENUM("ac203c DACL DACR Swap", sunxi_dac_swap_enum),

	/* DAP Func */
	SOC_ENUM_EXT("ac203c ADC HPF1 Mode", sunxi_adchpf1_sta_enum, ac203c_get_dap_status,
		     ac203c_set_dap_status),
	SOC_ENUM_EXT("ac203c ADC HPF2 Mode", sunxi_adchpf2_sta_enum, ac203c_get_dap_status,
		     ac203c_set_dap_status),
	SOC_ENUM_EXT("ac203c ADC HPF3 Mode", sunxi_adchpf3_sta_enum, ac203c_get_dap_status,
		     ac203c_set_dap_status),
	SOC_ENUM_EXT("ac203c DAC DRC Mode", sunxi_dacdrc_sta_enum, ac203c_get_dap_status,
		     ac203c_set_dap_status),
};
...
static const struct snd_soc_dapm_widget ac203c_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("ac203c ADC2 Input Src Mux", SND_SOC_NOPM, 0, 0, &adc2_input_src_mux),
	SND_SOC_DAPM_MUX("ac203c ADC3 Input Src Mux", SND_SOC_NOPM, 0, 0, &adc3_input_src_mux),

	SND_SOC_DAPM_MUX("ac203c ADC1 Data Select Mux", SND_SOC_NOPM, 0, 0, &adc1_src_mux),
	SND_SOC_DAPM_MUX("ac203c ADC2 Data Select Mux", SND_SOC_NOPM, 0, 0, &adc2_src_mux),
	SND_SOC_DAPM_MUX("ac203c ADC3 Data Select Mux", SND_SOC_NOPM, 0, 0, &adc3_src_mux),

	//i2s
	SND_SOC_DAPM_MUX("ac203c AIF TX1 Mux", SND_SOC_NOPM, 0, 0, &txm1_src_mux),
	SND_SOC_DAPM_MUX("ac203c AIF TX2 Mux", SND_SOC_NOPM, 0, 0, &txm2_src_mux),
	SND_SOC_DAPM_MUX("ac203c AIF TX3 Mux", SND_SOC_NOPM, 0, 0, &txm3_src_mux),

	SND_SOC_DAPM_MUX("ac203c RX1 Mux", SND_SOC_NOPM, 0, 0, &rxm1_src_mux),
	SND_SOC_DAPM_MUX("ac203c RX2 Mux", SND_SOC_NOPM, 0, 0, &rxm2_src_mux),
	SND_SOC_DAPM_MUX("ac203c RX3 Mux", SND_SOC_NOPM, 0, 0, &rxm3_src_mux),

	SND_SOC_DAPM_MUX("ac203c DACL Data Select Mux", SND_SOC_NOPM, 0, 0, &dacl_src_mux),
	SND_SOC_DAPM_MUX("ac203c DACR Data Select Mux", SND_SOC_NOPM, 0, 0, &dacr_src_mux),

	SND_SOC_DAPM_LINE("ac203c LINEOUTL", ac203c_lineoutl_event),
	SND_SOC_DAPM_LINE("ac203c LINEOUTR", ac203c_lineoutr_event),

	/* for pa */
	SND_SOC_DAPM_SPK("ac203c SPK", ac203c_spk_event),

	SND_SOC_DAPM_HP("ac203c HPOUT", ac203c_hpout_event),

	SND_SOC_DAPM_MIC("ac203c MIC1", ac203c_mic1_event),
	SND_SOC_DAPM_MIC("ac203c MIC2", ac203c_mic24_event),
	SND_SOC_DAPM_MIC("ac203c MIC3", ac203c_mic3_event),
	SND_SOC_DAPM_MIC("ac203c MIC4", ac203c_mic24_event),
};
...
static const struct snd_soc_dapm_route ac203c_dapm_routes[] = {
	...
	{"ac203c ADC2 Input Src Mux", "MIC2", "MICBIAS CHOP"},
	{"ac203c ADC2 Input Src Mux", "MIC4", "MICBIAS CHOP"},
	{"ac203c ADC2 Input Src Mux", "LINEOUTL", "LINEOUTLP_PIN"},
	{"ac203c ADC2 Input Src Mux", "LINEOUTL", "LINEOUTLN_PIN"},

	{"ac203c ADC3 Input Src Mux", "MIC3", "MICBIAS CHOP"},
	{"ac203c ADC3 Input Src Mux", "LINEOUTR", "LINEOUTRP_PIN"},
	{"ac203c ADC3 Input Src Mux", "LINEOUTR", "LINEOUTRN_PIN"},

	{"ac203c ADC1 Data Select Mux", "Debug_Data", "Debug_Data"},
	{"ac203c ADC1 Data Select Mux", "MIC1", "MICBIAS CHOP"},
	{"ac203c ADC1 Data Select Mux", "DACL_Data", "ac203c DACL Data Select Mux"},
	{"ac203c ADC1 Data Select Mux", "DACR_Data", "ac203c DACR Data Select Mux"},
	{"ac203c ADC1 Data Select Mux", "RXM1", "ac203c RX1 Mux"},

	{"ac203c ADC2 Data Select Mux", "Debug_Data", "Debug_Data"},
	{"ac203c ADC2 Data Select Mux", "ADC2_Input", "ac203c ADC2 Input Src Mux"},
	{"ac203c ADC2 Data Select Mux", "DACL_Data", "ac203c DACL Data Select Mux"},
	{"ac203c ADC2 Data Select Mux", "DACR_Data", "ac203c DACR Data Select Mux"},
	{"ac203c ADC2 Data Select Mux", "RXM1", "ac203c RX1 Mux"},

	{"ac203c ADC3 Data Select Mux", "Debug_Data", "Debug_Data"},
	{"ac203c ADC3 Data Select Mux", "ADC3_Input", "ac203c ADC3 Input Src Mux"},
	{"ac203c ADC3 Data Select Mux", "DACL_Data", "ac203c DACL Data Select Mux"},
	{"ac203c ADC3 Data Select Mux", "DACR_Data", "ac203c DACR Data Select Mux"},
	{"ac203c ADC3 Data Select Mux", "RXM1", "ac203c RX1 Mux"},

	{"ADC1", NULL, "ac203c ADC1 Data Select Mux"},
	{"ADC2", NULL, "ac203c ADC2 Data Select Mux"},
	{"ADC3", NULL, "ac203c ADC3 Data Select Mux"},

	{"ac203c AIF TX1 Mux", "ADC1_Data", "ADC1"},
	{"ac203c AIF TX1 Mux", "DACL_Data", "DACL"},
	{"ac203c AIF TX1 Mux", "ADC1_DACL_Data", "ADC1"},
	{"ac203c AIF TX1 Mux", "ADC1_DACL_Data", "DACL"},
	{"ac203c AIF TX1 Mux", "ADC1_DACL_Data_AVG", "ADC1"},
	{"ac203c AIF TX1 Mux", "ADC1_DACL_Data_AVG", "DACL"},

	{"ac203c AIF TX2 Mux", "ADC2_Data", "ADC2"},
	{"ac203c AIF TX2 Mux", "DACR_Data", "DACR"},
	{"ac203c AIF TX2 Mux", "ADC2_DACR_Data", "ADC2"},
	{"ac203c AIF TX2 Mux", "ADC2_DACR_Data", "DACR"},
	{"ac203c AIF TX2 Mux", "ADC2_DACR_Data_AVG", "ADC2"},
	{"ac203c AIF TX2 Mux", "ADC2_DACR_Data_AVG", "DACR"},

	{"ac203c AIF TX3 Mux", "ADC3_Data", "ADC3"},
	{"ac203c AIF TX3 Mux", "RXM1", "ac203c RX1 Mux"},
	{"ac203c AIF TX3 Mux", "ADC3_RXM1_Data", "ADC3"},
	{"ac203c AIF TX3 Mux", "ADC3_RXM1_Data", "ac203c RX1 Mux"},
	{"ac203c AIF TX3 Mux", "ADC3_RXM1_Data_AVG", "ADC3"},
	{"ac203c AIF TX3 Mux", "ADC3_RXM1_Data_AVG", "ac203c RX1 Mux"},

	{"AIF TX1", NULL, "ac203c AIF TX1 Mux"},
	{"AIF TX2", NULL, "ac203c AIF TX2 Mux"},
	{"AIF TX3", NULL, "ac203c AIF TX3 Mux"},

	/* dac route */
	{"ac203c RX1 Mux", "AIF_RXL", "AIF RXL"},
	{"ac203c RX1 Mux", "AIF_RXR", "AIF RXR"},
	{"ac203c RX1 Mux", "AIF_RXL_RXR", "AIF RXL"},
	{"ac203c RX1 Mux", "AIF_RXL_RXR", "AIF RXR"},
	{"ac203c RX1 Mux", "AIF_RXL_RXR_AVG", "AIF RXL"},
	{"ac203c RX1 Mux", "AIF_RXL_RXR_AVG", "AIF RXR"},

	{"ac203c RX2 Mux", "RXM1", "ac203c RX1 Mux"},
	{"ac203c RX2 Mux", "ADC1_Data", "ADC1"},
	{"ac203c RX2 Mux", "RXM1_ADC1", "ac203c RX1 Mux"},
	{"ac203c RX2 Mux", "RXM1_ADC1", "ADC1"},
	{"ac203c RX2 Mux", "RXM1_ADC1_AVG", "ac203c RX1 Mux"},
	{"ac203c RX2 Mux", "RXM1_ADC1_AVG", "ADC1"},

	{"ac203c RX3 Mux", "AIF_RXR", "AIF RXR"},
	{"ac203c RX3 Mux", "ADC2_Data", "ADC2"},
	{"ac203c RX3 Mux", "AIFRXR_ADC2", "AIF RXR"},
	{"ac203c RX3 Mux", "AIFRXR_ADC2", "ADC2"},
	{"ac203c RX3 Mux", "AIFRXR_ADC2_AVG", "AIF RXR"},
	{"ac203c RX3 Mux", "AIFRXR_ADC2_AVG", "ADC2"},

	{"ac203c DACL Data Select Mux", "RXM2", "ac203c RX2 Mux"},
	{"ac203c DACL Data Select Mux", "-6dB_Sine", "Debug_Data"},
	{"ac203c DACL Data Select Mux", "-60dB_Sine", "Debug_Data"},
	{"ac203c DACL Data Select Mux", "Zero", "Debug_Data"},

	{"ac203c DACR Data Select Mux", "RXM3", "ac203c RX3 Mux"},
	{"ac203c DACR Data Select Mux", "-6dB_Sine", "Debug_Data"},
	{"ac203c DACR Data Select Mux", "-60dB_Sine", "Debug_Data"},
	{"ac203c DACR Data Select Mux", "Zero", "Debug_Data"},

	{"DACL", NULL, "ac203c DACL Data Select Mux"},
	{"DACR", NULL, "ac203c DACR Data Select Mux"},
...
{
	size_t count_tmp = 0;
	struct ac203c_priv *ac203c = (struct ac203c_priv *)priv;

	if (!ac203c) {
		SND_LOG_ERR("priv to ac203c failed\n");
		return;
	}

	if (ac203c->dev->driver)
		if (ac203c->dev->driver->owner)
			goto module_version;
	return;

module_version:
	ac203c->module_version = ac203c->dev->driver->owner->version;
	count_tmp += sprintf(buf + count_tmp, "%s\n", ac203c->module_version);
...
static int snd_sunxi_dump_show(void *priv, char *buf, size_t *count)
{
	...
	if (!ac203c->show_reg_all)
		return 0;
	else
		ac203c->show_reg_all = false;

	for (i = 0; i < ARRAY_SIZE(sunxi_dig_reg_labels); ++i) {
		regmap_read(dig_regmap, sunxi_dig_reg_labels[i].address, &output_reg_val);
		count_tmp += sprintf(buf + count_tmp, "[0x%03x]: 0x%8x\n",
				     sunxi_dig_reg_labels[i].address, output_reg_val);
	}
...
static int snd_sunxi_dump_store(void *priv, const char *buf, size_t count)
{
	...
	if (input_reg_offset > SUNXI_AUDIO_DIG_MAX_REG) {
		pr_err("reg offset > audio max reg[0x%x]\n", SUNXI_AUDIO_DIG_MAX_REG);
		return -1;
	}
	regmap_read(dig_regmap, input_reg_offset, &output_reg_val);
	pr_info("reg[0x%03x]: 0x%x (old)\n", input_reg_offset, output_reg_val);
	regmap_write(dig_regmap, input_reg_offset, input_reg_val);
	regmap_read(dig_regmap, input_reg_offset, &output_reg_val);
...
#endif
{
	...
	/* clk init */
	ac203c->clk = snd_ac203c_clk_init(pdev);
	if (!ac203c->clk) {
		SND_LOG_ERR("clk init failed\n");
		ret = -EINVAL;
		goto err_snd_clk_init;
	}
	ret = snd_ac203c_clk_bus_enable(ac203c->clk);
	if (ret) {
...
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
	snprintf(ac203c->module_name, 32, "%s", "AC203C-DIG");
	dump = &ac203c->dump;
	dump->name = ac203c->module_name;
	dump->priv = ac203c;
	...
	SND_LOG_INFO("register ac203c platform success\n");
...
err_pin_init:
err_clk_bus_enable:
	snd_ac203c_clk_exit(ac203c->clk);
err_snd_clk_init:
...
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
	struct snd_sunxi_dump *dump = &ac203c->dump;
#endif
