...
#define SUNXI_MODNAME		"sound-ahub-i2s"
...
#define DRV_NAME	"sunxi-snd-plat-ahub-i2s"

unsigned int g_i2s_num;
				 ...
				 unsigned int freq_in, unsigned int freq_out)
{
	...
	if (dts->clk_mode && (freq_in % dts->clk_mode_mclk_freq)) {
		SND_LOG_ERR("pll(%d) and pre_pll(%d) not on the same freq\n",
...
static int sunxi_i2s_dai_set_sysclk(struct snd_soc_dai *dai, int clk_id, unsigned int freq, int dir)
{
	...
	if (dts->clk_mode && dts->clk_mode_mclk_freq && freq != dts->clk_mode_mclk_freq) {
...
static int sunxi_i2s_dai_set_bclk_ratio(struct snd_soc_dai *dai, unsigned int ratio)
{
	...
	if (dts->clk_mode && dts->clk_mode_lrck_freq && dts->clk_mode_bclk_freq) {
...
static int sunxi_i2s_dai_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	if (dts->clk_mode && dts->clk_mode_bclk_freq && dts->clk_mode_daifmt &&
	...
	/* set TDM format */
	ret = sunxi_i2s_set_daifmt_fmt(i2s, fmt & SND_SOC_DAIFMT_FORMAT_MASK);
	if (ret < 0) {
				      ...
				      int slots, int slot_width)
{
	...
	if (dts->clk_mode && dts->clk_mode_slots && (slots != dts->clk_mode_slots)
	    && dts->clk_mode_slot_width && (slot_width != dts->clk_mode_slot_width)) {
		SND_LOG_INFO("slots(%d) or slot_width(%d), using old_slot(%u) or"
			     "old_slot_width(%u)\n", slots, slot_width, dts->clk_mode_slots,
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (dts->clk_mode && dts->clk_mode_lrck_freq && sample_rate != dts->clk_mode_lrck_freq) {
		SND_LOG_INFO("sample_rate(%dHz) not same to pre_blck(%dHz, using pre_bclk)\n",
			     sample_rate, dts->clk_mode_lrck_freq);
		return -1;
	}

	if (dts->clk_mode && (dts->clk_mode_mclk_freq || dts->clk_mode_lrck_freq)) {
				   ...
				   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(i2s->i2s_ahub_rx, 16);
			snd_sunxi_set_apbif_fifo_mode(i2s->i2s_ahub_rx, 1);
			if (i2s->dts.dai_type == SUNXI_DAI_HDMI_TYPE &&
			    dma_params->hdmi_fmt > HDMI_FMT_PCM) {
				regmap_update_bits(regmap, SUNXI_I2S_FMT0,
						   0x7 << SAMPLE_RESOLUTION,
						   0x5 << SAMPLE_RESOLUTION);
				regmap_update_bits(regmap, SUNXI_I2S_FIFO_CTL,
						   0x1 << TXIM, 0x0 << TXIM);
			} else {
				regmap_update_bits(regmap, SUNXI_I2S_FMT0,
						   0x7 << SAMPLE_RESOLUTION,
						   0x3 << SAMPLE_RESOLUTION);
				regmap_update_bits(regmap, SUNXI_I2S_FIFO_CTL,
						   0x1 << TXIM, 0x0 << TXIM);
			}
		} else {
			snd_sunxi_set_apbif_sw(i2s->i2s_ahub_tx, 16);
			snd_sunxi_set_apbif_fifo_mode(i2s->i2s_ahub_tx, 1);
			regmap_update_bits(regmap, SUNXI_I2S_FMT0,
					   0x7 << SAMPLE_RESOLUTION,
					   0x3 << SAMPLE_RESOLUTION);
			regmap_update_bits(regmap, SUNXI_I2S_FIFO_CTL, 0x3 << RXOM, 0x0 << RXOM);
		}
		break;
	...
	case SNDRV_PCM_FORMAT_S32_LE:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(i2s->i2s_ahub_rx, 32);
			snd_sunxi_set_apbif_fifo_mode(i2s->i2s_ahub_rx, 0);
			regmap_update_bits(regmap, SUNXI_I2S_FIFO_CTL, 0x1 << TXIM, 0x0 << TXIM);
		} else {
			snd_sunxi_set_apbif_sw(i2s->i2s_ahub_tx, 32);
			snd_sunxi_set_apbif_fifo_mode(i2s->i2s_ahub_tx, 0);
			regmap_update_bits(regmap, SUNXI_I2S_FIFO_CTL, 0x3 << RXOM, 0x0 << RXOM);
		}
		break;
	default:
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_clk_gating_ctl(i2s->i2s_ahub_rx, true);
		snd_sunxi_set_apbif_ch(i2s->i2s_ahub_rx, params_channels(params));
		channel = snd_sunxi_ahub_ctl(i2s->i2s_ahub_rx, params_channels(params), true);
		snd_sunxi_apbif_ctl(i2s->i2s_ahub_rx, true);
	} else {
		snd_sunxi_clk_gating_ctl(i2s->i2s_ahub_tx, true);
		snd_sunxi_set_apbif_ch(i2s->i2s_ahub_tx, params_channels(params));
		channel = snd_sunxi_ahub_ctl(i2s->i2s_ahub_tx, params_channels(params), true);
		snd_sunxi_apbif_ctl(i2s->i2s_ahub_tx, true);
	}
	...
	/* set channels map */
	ret = sunxi_i2s_set_ch_map(i2s, channel);

	/* set channels */
	ret = sunxi_i2s_set_ch_en(i2s, substream->stream, channel);
	if (ret < 0) {
...
exit:
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_ahub_ctl(i2s->i2s_ahub_rx, 0, false);
		snd_sunxi_apbif_ctl(i2s->i2s_ahub_rx, false);
		snd_sunxi_clk_gating_ctl(i2s->i2s_ahub_rx, false);
	} else {
		snd_sunxi_ahub_ctl(i2s->i2s_ahub_tx, 0, false);
		snd_sunxi_apbif_ctl(i2s->i2s_ahub_tx, false);
		snd_sunxi_clk_gating_ctl(i2s->i2s_ahub_tx, false);
	}
...
static int sunxi_i2s_dai_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		for (i = 0; i < 10; i++) {
			snd_sunxi_flush_fifo(i2s->i2s_ahub_rx);
			mdelay(1);
		}
		regmap_write(regmap, SUNXI_I2S_TXCNT, 0);
	} else {
		snd_sunxi_flush_fifo(i2s->i2s_ahub_tx);
...
static void sunxi_i2s_dai_rx_route(struct sunxi_i2s *i2s, bool enable)
{
	...
	if (enable) {
		...
		snd_sunxi_drq_ctl(i2s->i2s_ahub_tx, true);
	} else {
		snd_sunxi_drq_ctl(i2s->i2s_ahub_tx, false);
				 ...
				 int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK && i2s->audio_sta.spk)
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK && i2s->audio_sta.spk)
			snd_sunxi_pa_pin_disable_irp(i2s->pa_cfg, i2s->pa_pin_max, NULL, NULL);
		fallthrough;
...
#define CREATE_I2S_KCONTROLS(n) \
	static const struct snd_kcontrol_new sunxi_i2s_controls##n[] = { \
		SOC_ENUM_EXT("i2s" #n " rx sync mode", sunxi_rx_sync_mode_enum, \
			     sunxi_get_rx_sync_mode, sunxi_set_rx_sync_mode), \
		SOC_ENUM_EXT("i2s" #n " tx trigger bypass", sunxi_tx_trigger_mode_enum, \
			     sunxi_get_tx_trigger_mode, sunxi_set_tx_trigger_mode), \
		SOC_SINGLE("i2s" #n " loopback debug", SUNXI_I2S_CTL, LOOP_EN, 1, 0), \
		SOC_SINGLE_EXT("i2s" #n " clk en post delay", SND_SOC_NOPM, 0, 5000, 0, \
			       sunxi_get_i2s_delay_mode, \
			       sunxi_put_i2s_delay_mode), \
	};

#define CREATE_I2S_DAPM(n) \
	static const struct snd_soc_dapm_widget sunxi_i2s_dapm_widgets##n[] = { \
		SND_SOC_DAPM_AIF_IN("DAC" #n, "Playback", 0, SND_SOC_NOPM, 0, 0), \
		SND_SOC_DAPM_OUTPUT("I2S_PIN"#n), \
		SND_SOC_DAPM_SPK("I2S" #n " _SPK", sunxi_spk_event), \
	}; \
	static const struct snd_soc_dapm_route sunxi_i2s_dapm_routes##n[] = { \
		{"I2S_PIN" #n, NULL, "DAC" #n}, \
	};
...
static int sunxi_i2s_component_probe(struct snd_soc_component *component)
{
	...
	for (i = 0; i < i2s->reg_group.size; ++i)
		reg_label[i].address = audio_reg_addrs[i];
...
#define CREATE_I2S_DAI_DRIVER(n) \
	static struct snd_soc_dai_driver sunxi_i2s_dai##n = { \
		.name = DRV_NAME#n, \
		.playback = { \
...
#define CREATE_I2S_COMPONENT_DRIVER(n) \
	static struct snd_soc_component_driver sunxi_i2s_component##n = { \
		.name			= DRV_NAME#n, \
		...
		.controls		= sunxi_i2s_controls##n, \
		.num_controls		= ARRAY_SIZE(sunxi_i2s_controls##n), \
		.dapm_widgets		= sunxi_i2s_dapm_widgets##n, \
		.num_dapm_widgets	= ARRAY_SIZE(sunxi_i2s_dapm_widgets##n), \
		.dapm_routes		= sunxi_i2s_dapm_routes##n, \
		.num_dapm_routes	= ARRAY_SIZE(sunxi_i2s_dapm_routes##n), \
	};

#define CREATE_I2S_SETTING(n) \
	CREATE_I2S_KCONTROLS(n) \
	CREATE_I2S_DAPM(n) \
	CREATE_I2S_DAI_DRIVER(n) \
	CREATE_I2S_COMPONENT_DRIVER(n)

CREATE_I2S_SETTING(0)
CREATE_I2S_SETTING(1)
CREATE_I2S_SETTING(2)
CREATE_I2S_SETTING(3)
CREATE_I2S_SETTING(4)

static struct snd_soc_dai_driver *sunxi_i2s_dais[] = {
	&sunxi_i2s_dai0,
	&sunxi_i2s_dai1,
	&sunxi_i2s_dai2,
	&sunxi_i2s_dai3,
...
static struct snd_soc_component_driver *sunxi_i2s_components[] = {
	&sunxi_i2s_component0,
	&sunxi_i2s_component1,
	&sunxi_i2s_component2,
	&sunxi_i2s_component3,
	&sunxi_i2s_component4,
};
...
static unsigned int snd_sunxi_dts_parse_daifmt(struct platform_device *pdev, struct sunxi_i2s *i2s)
{
	struct device_node *np = pdev->dev.of_node;
...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_i2s *i2s)
{
	...
	if (dts->clk_mode) {
		...
		if (!dts->clk_mode_bclk_freq && dts->clk_mode_lrck_freq) {
...
static int sunxi_i2s_dai_set_fmt_pre(struct sunxi_i2s *i2s, unsigned int fmt)
{
	struct regmap *regmap = i2s->mem.regmap;
	unsigned int lrck_polarity, bclk_polarity;
	/* dai mode of i2s format
...
static int sunxi_i2s_pre_open_clk(struct sunxi_i2s *i2s)
{
	...
	unsigned int bclk_ratio, bclk_ratio_map;
	unsigned int slot_width_map;
	...
	/* bclk */
	bclk_ratio = freq / (dts->clk_mode_lrck_freq * dts->clk_mode_slots * dts->clk_mode_slot_width);
	for (i = 0; i < ARRAY_SIZE(sunxi_i2s_mclk_bclk_div); i++) {
	...
	regmap_update_bits(regmap, SUNXI_I2S_CLKDIV, 0xf << BCLK_DIV, bclk_ratio_map << BCLK_DIV);
...
static void snd_sunxi_dma_params_init(struct sunxi_i2s *i2s)
{
	...
	i2s->playback_dma_param.dma_addr = snd_sunxi_get_dev_fifo(i2s->i2s_ahub_rx);
	...
	i2s->capture_dma_param.dma_addr = snd_sunxi_get_dev_fifo(i2s->i2s_ahub_tx);
...
static int sunxi_i2s_dev_probe(struct platform_device *pdev)
{
	...
	i2s->i2s_ahub_tx = SUNXI_AHUB_PORT_I2S0_TX + g_i2s_num;
	i2s->i2s_ahub_rx = SUNXI_AHUB_PORT_I2S0_RX + g_i2s_num;

	/* pa_pin init */
	i2s->pa_cfg = snd_sunxi_pa_pin_init(pdev, &i2s->pa_pin_max);

	reg_label = devm_kzalloc(dev, sizeof(*reg_label) * ARRAY_SIZE(audio_reg_addrs),
				 GFP_KERNEL);
	...
	i2s->reg_group.size = ARRAY_SIZE(audio_reg_addrs);
	...
	sunxi_adpt_set_dai_ops(sunxi_i2s_dais[g_i2s_num], &sunxi_i2s_dai_ops, &priv);

	ret = snd_soc_register_component(&pdev->dev, sunxi_i2s_components[g_i2s_num],
					 sunxi_i2s_dais[g_i2s_num], 1);
	if (ret) {
	...
	if (dts->clk_mode)
		sunxi_i2s_pre_open_clk(i2s);

	/* soft reset */
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_I2S0_TX + g_i2s_num);
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_I2S0_RX + g_i2s_num);

#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
	snprintf(i2s->module_name, 32, "%s%u", "I2S", g_i2s_num);
	...
	i2s->i2s_num = g_i2s_num++;
	SND_LOG_INFO("register i2s%u platform success\n", i2s->i2s_num);
...
static int sunxi_i2s_dev_remove(struct platform_device *pdev)
{
	...
	SND_LOG_DEBUG("unregister i2s%u platform success\n", g_i2s_num);
...
MODULE_DESCRIPTION("sunxi soundcard platform of ahub i2s");
