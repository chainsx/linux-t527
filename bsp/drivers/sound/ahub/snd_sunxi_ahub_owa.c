...
#define SUNXI_MODNAME		"sound-ahub-owa"
...
#include "snd_sunxi_ahub_owa.h"
#include "snd_sunxi_adapter.h"

#define DRV_NAME	"sunxi-snd-plat-ahub-owa"
...
static struct audio_reg_label sunxi_reg_labels[] = {
	...
	REG_LABEL(SUNXI_OWA_INS_REDET_CTL),
	...
	REG_LABEL(SUNXI_OWA_EXP_CDR_DBG),
};
...
static void sunxi_owa_lock_confirm_work(struct work_struct *work)
{
	...
	if (owa_flag_state & (0x1 << RXCFG_LOCK_FLAG)) {
		...
		/* flush RXFIFO */
		snd_sunxi_flush_fifo(SUNXI_AHUB_PORT_OWA_TX);
		regmap_write(regmap, SUNXI_OWA_RXCNT, 0);

		/* enable RXDRQEN & RXEN */
		regmap_update_bits(regmap, SUNXI_OWA_RXCFG, 1 << RXCFG_CHST_CP, 1 << RXCFG_CHST_CP);
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_OWA_TX, true);
		...
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_OWA_TX, false);
				   ...
				   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_OWA_RX, 16);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_OWA_RX, 1);
		} else {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_OWA_TX, 16);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_OWA_TX, 1);
		}
		reg_temp = 0;
		tx_input_mode = 0;
		rx_output_mode = 0;
	...
	case SNDRV_PCM_FORMAT_S32_LE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_OWA_RX, 32);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_OWA_RX, 0);
		} else {
			snd_sunxi_set_apbif_sw(SUNXI_AHUB_PORT_OWA_TX, 32);
			snd_sunxi_set_apbif_fifo_mode(SUNXI_AHUB_PORT_OWA_TX, 0);
		}
		reg_temp = 2;
		tx_input_mode = 0;
		rx_output_mode = 0;
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_OWA_RX, true);
		snd_sunxi_set_apbif_ch(SUNXI_AHUB_PORT_OWA_RX, params_channels(params));
		channel = snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_OWA_RX, params_channels(params), true);
		regmap_update_bits(regmap, SUNXI_OWA_CHAN_CFG,
				   0xF << CHANCFG_TX_CHAN_NUM,
				   (channel - 1) << CHANCFG_TX_CHAN_NUM);
		if (!channel)
		...
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_OWA_RX, true);
	} else {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_OWA_TX, true);
		snd_sunxi_set_apbif_ch(SUNXI_AHUB_PORT_OWA_TX, params_channels(params));
		channel = snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_OWA_TX, params_channels(params), true);
		regmap_update_bits(regmap, SUNXI_OWA_CHAN_CFG,
				   0xF << CHANCFG_RX_CHAN_NUM,
				   (channel - 1) << CHANCFG_RX_CHAN_NUM);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_OWA_TX, true);
	}
...
static int sunxi_owa_dai_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_OWA_RX, false);
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_OWA_RX, 0, false);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_OWA_RX, false);
	} else {
		snd_sunxi_clk_gating_ctl(SUNXI_AHUB_PORT_OWA_TX, false);
		snd_sunxi_ahub_ctl(SUNXI_AHUB_PORT_OWA_TX, 0, false);
		snd_sunxi_apbif_ctl(SUNXI_AHUB_PORT_OWA_TX, false);
	}
...
static int sunxi_owa_dai_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_sunxi_flush_fifo(SUNXI_AHUB_PORT_OWA_RX);
	...
	regmap_update_bits(regmap, SUNXI_OWA_CTL,
			   1 << CTL_RST_TX | 1 << CTL_GEN_EN, 1 << CTL_RST_TX | 1 << CTL_GEN_EN);
...
static void sunxi_owa_txctrl_enable(struct sunxi_owa *owa, int enable)
{
	...
	if (enable) {
		...
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_OWA_RX, true);
	} else {
		regmap_update_bits(regmap, SUNXI_OWA_TXCFG, 1 << TXCFG_TXEN, 0 << TXCFG_TXEN);
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_OWA_RX, false);
	}
}
...
static void sunxi_owa_rxctrl_enable(struct sunxi_owa *owa, int enable)
{
	...
	if (enable) {
		regmap_update_bits(regmap, SUNXI_OWA_RXCFG,
				   1 << RXCFG_CHST_CP, 1 << RXCFG_CHST_CP);
		snd_sunxi_drq_ctl(SUNXI_AHUB_PORT_OWA_TX, true);
		regmap_update_bits(regmap, SUNXI_OWA_RXCFG, 1 << RXCFG_RXEN, 1 << RXCFG_RXEN);
		if (owa_irq->id) {
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	regmap_update_bits(regmap, SUNXI_OWA_TXCH_STA0,
			   1 << TXCHSTA0_DATA_TYPE, reg_val << TXCHSTA0_DATA_TYPE);
	regmap_update_bits(regmap, SUNXI_OWA_RXCH_STA0,
			   1 << RXCHSTA0_DATA_TYPE, reg_val << RXCHSTA0_DATA_TYPE);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	case RX_DATA_TYPE_SHIFT:
		ucontrol->value.integer.value[0] = (reg_val_0 & (0x1 << RXCHSTA0_DATA_TYPE)) ? 1 : 0;
		break;
	default:
		return -EINVAL;
	}
...
static const struct snd_kcontrol_new sunxi_rx_sync_controls[] = {
	SOC_ENUM_EXT("owa rx sync mode", sunxi_rx_sync_mode_enum,
		     sunxi_get_rx_sync_mode, sunxi_set_rx_sync_mode),
};

static const struct snd_kcontrol_new sunxi_loopback_controls[] = {
	SOC_SINGLE("owa loopback debug", SUNXI_OWA_CTL, CTL_LOOP_EN, 1, 0),
};

static const struct snd_kcontrol_new sunxi_owa_raw_controls[] = {
	SOC_ENUM_EXT("owa audio tx data format", tx_data_fmt_enum,
		     sunxi_owa_get_tx_data_fmt, sunxi_owa_set_tx_data_fmt),
	SOC_ENUM_EXT("owa audio rx data format", rx_data_fmt_enum,
		     sunxi_owa_get_rx_data_fmt, sunxi_owa_set_rx_data_fmt),
};

static const struct snd_kcontrol_new sunxi_debug_controls[] = {
	SOC_ENUM_EXT("owa rx channel cnt", sunxi_rx_ch_cnt_enum,
		     sunxi_get_rx_debug_info, sunxi_set_null_value),
	SOC_ENUM_EXT("owa rx word length", sunxi_word_len_enum,
		     sunxi_get_rx_debug_info, sunxi_set_null_value),
	SOC_ENUM_EXT("owa rx rate", sunxi_rate_enum,
		     sunxi_get_rx_debug_info, sunxi_set_null_value),
	SOC_ENUM_EXT("owa rx origin rate", sunxi_origin_rate_enum,
		     sunxi_get_rx_debug_info, sunxi_set_null_value),
	SOC_ENUM_EXT("owa rx data type", sunxi_rx_data_type_enum,
		     sunxi_get_rx_debug_info, sunxi_set_null_value),
};
...
static int sunxi_owa_component_probe(struct snd_soc_component *component)
{
	...
	ret = snd_soc_add_component_controls(component, sunxi_owa_raw_controls,
					     ARRAY_SIZE(sunxi_owa_raw_controls));
	if (ret)
		SND_LOG_ERR_STD(E_OWA_SWSYS_COMP_PROBE, "add raw kcontrols failed\n");
...
static void snd_sunxi_dma_params_init(struct sunxi_owa *owa)
{
	...
	owa->playback_dma_param.dma_addr = snd_sunxi_get_dev_fifo(SUNXI_AHUB_PORT_OWA_RX);
	...
	owa->capture_dma_param.dma_addr = snd_sunxi_get_dev_fifo(SUNXI_AHUB_PORT_OWA_TX);
...
static int sunxi_owa_dev_probe(struct platform_device *pdev)
{
	...
	/* soft reset */
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_OWA_TX);
	snd_sunxi_soft_reset(SUNXI_AHUB_PORT_OWA_RX);
	...
	SND_LOG_INFO("register owa platform success\n");
...
MODULE_DESCRIPTION("sunxi soundcard platform of ahub owa");
