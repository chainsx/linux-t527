...
#define SUNXI_MODNAME		"sound-ahub-mach"
...
#include "snd_sunxi_ahub_mach_utils.h"

#define DAI		"sound-dai"
#define CELL		"#sound-dai-cells"
#define PREFIX		"soundcard-mach,"

#define DRV_NAME	"sunxi-snd-ahub-mach"

static LIST_HEAD(module_list);
static unsigned int g_module_cnt;
...
{
	size_t count_tmp = 0;
	struct asoc_simple_dai *dai = (struct asoc_simple_dai *)priv_orig;
	struct asoc_simple_priv *priv = (struct asoc_simple_priv *)(dai->priv);
...
module_version:
	priv->dump[dai->index].module_version = priv->snd_card.dev->driver->owner->version;
	count_tmp += sprintf(buf + count_tmp, "%s\n", priv->dump[dai->index].module_version);
...
static int snd_sunxi_dump_show(void *priv_orig, char *buf, size_t *count)
{
	...
	unsigned int dai_fmt		= card->dai_link[dai->index].dai_fmt;
	struct snd_sunxi_dai_ucfmt *dai_ucfmt = &priv->dai_props[dai->index].dai_ucfmt;
	unsigned int mclk_fs		= priv->dai_props[dai->index].mclk_fs;
	unsigned int *mclk_fp		= priv->dai_props[dai->index].mclk_fp;
	unsigned int cpu_pll_fs		= priv->dai_props[dai->index].cpu_pll_fs;
	unsigned int codec_pllin_fs	= priv->dai_props[dai->index].codec_pllin_fs;
	unsigned int codec_pllout_fs = priv->dai_props[dai->index].codec_pllout_fs;
	int slots			= dai->slots;
	int slot_width			= dai->slot_width;
	char prop[8] = {0};

	if (!priv->dump[dai->index].show_daifmt)
		return 0;
	else
		priv->dump[dai->index].show_daifmt = false;
...
static int snd_sunxi_dump_store(void *priv_orig, const char *buf, size_t count)
{
	struct asoc_simple_dai *dai = (struct asoc_simple_dai *)priv_orig;
	struct asoc_simple_priv *priv = (struct asoc_simple_priv *)(dai->priv);
	struct simple_dai_props *dai_props = simple_priv_to_props(priv, dai->index);
	...
	unsigned int mclk_fs		= priv->dai_props[dai->index].mclk_fs;
	unsigned int mclk_fp0		= priv->dai_props[dai->index].mclk_fp[0];
	unsigned int mclk_fp1		= priv->dai_props[dai->index].mclk_fp[1];
	unsigned int cpu_pll_fs		= priv->dai_props[dai->index].cpu_pll_fs;
	unsigned int codec_pllin_fs	= priv->dai_props[dai->index].codec_pllin_fs;
	unsigned int codec_pllout_fs	= priv->dai_props[dai->index].codec_pllout_fs;
	...
	if (scanf_num == 0) {
		priv->dump[dai->index].show_daifmt = true;
		return 0;
	}
	...
	if (scanf_num > 0) {
		...
		card->dai_link[dai->index].dai_fmt		= dai_fmt;
		priv->dai_props[dai->index].mclk_fs		= mclk_fs;
		priv->dai_props[dai->index].mclk_fp[0]		= mclk_fp0;
		priv->dai_props[dai->index].mclk_fp[1]		= mclk_fp1;
		priv->dai_props[dai->index].cpu_pll_fs		= cpu_pll_fs;
		priv->dai_props[dai->index].codec_pllin_fs	= codec_pllin_fs;
		priv->dai_props[dai->index].codec_pllout_fs	= codec_pllout_fs;
		dai->slots					= slots;
		dai->slot_width					= slot_width;
...
static int simple_ahub_soc_probe(struct snd_soc_card *card)
{
	struct asoc_simple_priv *priv = snd_soc_card_get_drvdata(card);
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
...
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
	for (i = 0; i < g_module_cnt; i++) {
		if (asoc_simple_is_i2sdai(&priv->dais[i])) {
			priv->dais[i].index = i;
			priv->dais[i].priv = priv;
			dump = &priv->dump[i].dump;
			snprintf(priv->dump[i].module_name, 32, "%s-i2s%d", "machine",
				 priv->dais[i].id);
			dump->name = priv->dump[i].module_name;
			dump->priv = &priv->dais[i];
...
static int simple_ahub_soc_remove(struct snd_soc_card *card)
{
	struct asoc_simple_priv *priv = snd_soc_card_get_drvdata(card);
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_DEBUG)
	int i;
	...
	for (i = 0; i < g_module_cnt; i++) {
		if (asoc_simple_is_i2sdai(&priv->dais[i]))
			snd_sunxi_dump_unregister(&priv->dump[i].dump);
	}
#endif
				 ...
				 struct snd_pcm_hw_params *params)
{
	...
	struct asoc_simple_dai *dais = simple_priv_to_dais(priv, RTD_NUM(rtd));
	unsigned int mclk = 0;
...
static int simple_dai_link_of(struct device_node *node, struct asoc_simple_priv *priv)
{
	...
	ret = asoc_simple_parse_i2s_id(top_np, prefix, priv->dais);
	if (ret < 0)
		goto dai_link_of_err;
...
// Based on - simple_dai_link_of
static int simple_ahub_dai_link_of(struct device_node *node, struct asoc_simple_priv *priv,
				   int index)
{
	struct device *dev = simple_priv_to_dev(priv);
	struct snd_soc_dai_link *dai_link = simple_priv_to_link(priv, index);
	struct simple_dai_props *dai_props = simple_priv_to_props(priv, index);
	struct asoc_simple_dai *dais = simple_priv_to_dais(priv, index);
	struct device_node *top_np = NULL;
	struct device_node *cpu = NULL;
	struct device_node *plat = NULL;
	struct device_node *codec = NULL;
	...
	ret = asoc_simple_parse_i2s_id(top_np, prefix, dais);
	if (ret < 0)
		goto dai_link_of_err;

	ret = asoc_simple_parse_tdm_slot(top_np, prefix, dais);
	if (ret < 0)
		goto dai_link_of_err;
...
// Based on - simple_parse_of
static int sunxi_ahub_parse_of(struct asoc_simple_priv *priv)
{
	struct sunxi_ahub_mach *ahub_mach, *n;
	struct device *dev = simple_priv_to_dev(priv);
	struct snd_soc_card *card = simple_priv_to_card(priv);
	struct device_node *top_np = dev->of_node;
	int ret, i = 0;
	...
	list_for_each_entry_safe(ahub_mach, n, &module_list, list) {
		/* For single DAI link & old style of DT node */
		ret = simple_ahub_dai_link_of(ahub_mach->node, priv, i++);
		if (ret < 0) {
			SND_LOG_ERR("simple_ahub_dai_link_of failed\n");
			return ret;
		}
	}
...
static int sunxi_ahub_register_card(struct device *dev, bool single_interface)
{
	...
	/* Initialization of sound card resources */
	priv = devm_kzalloc(dev, sizeof(*priv), GFP_KERNEL);
	if (!priv) {
		SND_LOG_ERR("alloc failed\n");
		return -ENOMEM;
	}
	...
	if (single_interface) {
		...
		if (ret < 0) {
			SND_LOG_ERR("init priv failed\n");
			goto err;
		}

		ret = simple_parse_of(priv);
		if (ret < 0) {
			SND_LOG_ERR("parse dts failed\n");
			goto err;
		}
	} else {
		card->probe	= simple_ahub_soc_probe;
		card->remove	= simple_ahub_soc_remove;

		ret = asoc_ahub_simple_init_priv(priv, g_module_cnt);
		if (ret < 0) {
			SND_LOG_ERR("init priv failed\n");
			goto err;
		}

		ret = sunxi_ahub_parse_of(priv);
		if (ret < 0) {
			SND_LOG_ERR("parse dts failed\n");
			goto err;
		}
	...
	if (ret < 0) {
		SND_LOG_ERR("register card failed\n");
		goto err;
	}
...
static int asoc_machine_dev_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;

	struct sunxi_ahub_mach *ahub_mach;
	unsigned int temp_val;
	int ret;

	SND_LOG_DEBUG("\n");

	ret = of_property_read_u32(np, "soundcard-audiohub", &temp_val);
	if (ret == 0 && temp_val == 1) {
		/* Add the module to the linked list */
		if (np && of_device_is_available(np)) {
			ahub_mach = devm_kzalloc(dev, sizeof(*ahub_mach), GFP_KERNEL);
			if (!ahub_mach) {
				SND_LOG_ERR("alloc failed\n");
				return -ENOMEM;
			}

			INIT_LIST_HEAD(&ahub_mach->list);
			ahub_mach->dev = dev;
			ahub_mach->node = np;
			list_add_tail(&ahub_mach->list, &module_list);
			g_module_cnt++;
			return 0;
		} else {
			SND_LOG_ERR("machine dts available\n");
			goto err;
		}
	} else if (ret == 0 && temp_val == 0) {
		/* Register dam control sound card */
		ret = sunxi_ahub_register_card(dev, true);
		if (ret) {
			SND_LOG_ERR("register card failed\n");
			goto err;
		}
	} else if (ret < 0) {
		/* Register all the nodes in the linked list to the sound card */
		ret = sunxi_ahub_register_card(dev, false);
		if (ret) {
			SND_LOG_ERR("register card failed\n");
			goto err;
		}
	...
	return 0;
err:
	devm_kfree(dev, ahub_mach);
	return -1;
}

static int asoc_machine_dev_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct sunxi_ahub_mach *ahub_mach, *n;
...
static struct platform_driver sunxi_soundcard_machine_driver = {
	...
	.probe	= asoc_machine_dev_probe,
	.remove	= asoc_machine_dev_remove,
};

int __init sunxi_soundcard_machine_ahub_dev_init(void)
{
	int ret;
...
void __exit sunxi_soundcard_machine_ahub_dev_exit(void)
{
	platform_driver_unregister(&sunxi_soundcard_machine_driver);
}

late_initcall(sunxi_soundcard_machine_ahub_dev_init);
module_exit(sunxi_soundcard_machine_ahub_dev_exit);

MODULE_AUTHOR("huhaoxin@allwinnertech.com");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0.1");
MODULE_DESCRIPTION("sunxi soundcard ahub machine");