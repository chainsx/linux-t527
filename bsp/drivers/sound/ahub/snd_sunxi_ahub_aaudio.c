...
#define SUNXI_MODNAME		"sound-ahub-aaudio"
...
#define DRV_NAME	"sunxi-snd-plat-ahub-aaudio"
...
static int sunxi_aaudio_parse_dma_param(struct device_node *np, struct sunxi_cpudai *sunxi_cpudai)
{
	...
	sunxi_cpudai->playback_dma_param.dma_addr =
		snd_sunxi_get_dev_fifo(SUNXI_AHUB_PORT_CODEC_RX);
	...
	sunxi_cpudai->capture_dma_param.dma_addr =
		snd_sunxi_get_dev_fifo(SUNXI_AHUB_PORT_CODEC_TX);
	...
	SND_LOG_DEBUG("dac-fifo   : 0x%llx\n", sunxi_cpudai->playback_dma_param.dma_addr);
	SND_LOG_DEBUG("adc-fifo   : 0x%llx\n", sunxi_cpudai->capture_dma_param.dma_addr);
#else
	SND_LOG_DEBUG("dac-fifo   : 0x%x\n", sunxi_cpudai->playback_dma_param.dma_addr);
	SND_LOG_DEBUG("adc-fifo   : 0x%x\n", sunxi_cpudai->capture_dma_param.dma_addr);
#endif
...
static int sunxi_aaudio_dev_probe(struct platform_device *pdev)
{
	...
	SND_LOG_INFO("register aaudio platform success\n");
...
MODULE_DESCRIPTION("sunxi soundcard platform of ahub aaudio");
