...
#define SUNXI_MODNAME		"sound-ahub-mach-utils"
...
// Based on - asoc_simple_init_priv
int asoc_ahub_simple_init_priv(struct asoc_simple_priv *priv, unsigned int dai_link_cnt)
{
	...
	dai_props = devm_kcalloc(dev, dai_link_cnt, sizeof(*dai_props), GFP_KERNEL);
	dai_link  = devm_kcalloc(dev, dai_link_cnt, sizeof(*dai_link),  GFP_KERNEL);
	dais      = devm_kcalloc(dev, dai_link_cnt, sizeof(*dais),      GFP_KERNEL);
	if (!dai_props || !dai_link || !dais)
		return -ENOMEM;
			     ...
			     struct asoc_simple_dai *dais)
{
	...
	snprintf(prop, sizeof(prop), "%si2s-id", prefix);
	ret = of_property_read_u32(node, prop, &val);
	if (!ret)
		dais->id = val;
	else
