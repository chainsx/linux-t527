...
static int aicwf_resume_access(void)
{
	...
	if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80)
		aicwf_8800d80_fw_init(rwnx_hw);
	else if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80X2)
		aicwf_8800d80x2_fw_init(rwnx_hw);
...
static int aicwf_pcie_init(struct aic_pci_dev *pciedev)
{
	...
	case PRODUCT_ID_AIC8800D80:
		pciedev->pci_bar0_vaddr = (u8 *)pci_ioremap_bar(pci_dev, 0);
		...
		pciedev->pci_bar1_vaddr = (u8 *)pci_ioremap_bar(pci_dev, 1);
		...
		pciedev->pci_bar2_vaddr = (u8 *)pci_ioremap_bar(pci_dev, 2);
	...
	case PRODUCT_ID_AIC8800D80X2:
		...
		adev->map0 = (u8 *)pci_ioremap_bar(adev->pdev, 0);
		if (!adev->map0) {
			dev_err(&(pci_dev->dev), "pci_ioremap_bar0 failed\n");
			ret = -ENODEV;
			goto out_bar0;
