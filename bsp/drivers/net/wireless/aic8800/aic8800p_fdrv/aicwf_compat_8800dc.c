...
{
	if (rwnx_send_txpwr_lvl_req(rwnx_hw)) {
		return -1;
	}

	if (rwnx_send_txpwr_ofst_req(rwnx_hw)) {
		return -1;
	}


	if (testmode == 0) {
		if (rwnx_send_rf_config_req(rwnx_hw, 0,	1, (u8_l *)txgain_table, 128))
			return -1;

		if (rwnx_send_rf_config_req(rwnx_hw, 0,	0, (u8_l *)rxgain_table_24g_20m, 256))
			return -1;

		if (rwnx_send_rf_config_req(rwnx_hw, 32,  0, (u8_l *)rxgain_table_24g_40m, 256))
			return -1;
