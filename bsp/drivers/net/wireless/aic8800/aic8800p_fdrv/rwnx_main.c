...
char aic_fw_path_8800d80x2[200] = "/lib/firmware";
u8 chip_sub_id = 0;
u8 chip_mcu_id = 0;
u8 chip_id = 0;
...
//close lp mode
	...
	if (rwnx_hw->vif_started == 0) {
		...
		error = rwnx_send_rf_calib_req(rwnx_hw, &cfm);
		if (error) {
			return -1;
		}
		#endif
...
#else
	if (found)
		fg80211_probe_status(rwnx_vif->ndev, mac, (u64)rwnx_vif->sta_probe.probe_id, 1, GFP_ATOMIC);
	else
...
int rwnx_ic_rf_init(struct rwnx_hw *rwnx_hw)
{
	...
	if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80)
		...
		ret = aicwf_set_rf_config_8800d80x2(rwnx_hw, &cfm);
	if (ret)
		return -1;
