...
//	{USB_VENDOR_ID_AIC,     USB_DEVICE_ID_AIC_8800D80,   PRODUCT_ID_AIC8800D80,   "aic8800d80",   0, 0}, // 8800d80 in bootloader mode
	...
	{USB_VENDOR_ID_AIC,     USB_DEVICE_ID_AIC_8800DC,    PRODUCT_ID_AIC8800DC,    "aic8800dc",    0, 0},
	{USB_VENDOR_ID_AIC,     USB_DEVICE_ID_AIC_8800DW,    PRODUCT_ID_AIC8800DW,    "aic8800dw",    0, 0},
		...
		if (urb->status < 0) {
			...
			if (g_rwnx_plat->wait_disconnect_cb == false) {
				...
				if (atomic_read(&aicwf_deinit_atomic) > 0) {
					...
					usb_err("%s need to wait for disconnect callback \r\n",
						__func__);
				} else {
			...
			if ((type & USB_TYPE_CFG) != USB_TYPE_CFG) {
				...
				if (type == USB_TYPE_CFG_CMD_RSP) {
					...
					if ((pkt_total_len > AICWF_USB_MSG_MAX_PKT_SIZE) && (skb->len == AICWF_USB_MSG_MAX_PKT_SIZE)) {
						usb_err("reassemble msg pkt, len=%u\n", pkt_total_len);
...
#endif  /* CONFIG_USB_RX_REASSEMBLE */
...
static int aicwf_usb_submit_rx_urb(struct aic_usb_dev *usb_dev, struct aicwf_usb_buf *usb_buf)
{
	int ret;
	...
	if (rx_buff == NULL) {
		usb_err("failed to alloc rxbuff\r\n");
		aicwf_usb_rx_buf_put(usb_dev, usb_buf);
		rx_urb_sched = true;
		return -1;
	}
				...
				struct aicwf_usb_buf *usb_buf)
{
	...
	usb_fill_bulk_urb(usb_buf->urb, usb_dev->udev, usb_dev->msg_in_pipe, skb->data,
			  AICWF_USB_MSG_MAX_PKT_SIZE, aicwf_usb_msg_rx_complete, usb_buf);
...
static struct usb_device_id aicwf_usb_id_table[] = {
	...
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_AIC, USB_DEVICE_ID_AIC_8800DC, 0xff, 0xff, 0xff)},
	{USB_DEVICE(USB_VENDOR_ID_AIC, USB_DEVICE_ID_AIC_8800DW)},
