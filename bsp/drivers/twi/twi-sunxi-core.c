...
#define SUNXI_TWI_VERSION	"2.8.12"
...
static struct sunxi_twi_hw_data sunxi_twi_v100_data = {
	...
	.drv_timeout_n_offest = 8,
};

static struct sunxi_twi_hw_data sunxi_twi_v101_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30 | USE_CLK_DUTY_30,
	.slave_func_fixed = false,
	.drv_timeout_n_offest = 8,
};

static struct sunxi_twi_hw_data sunxi_twi_v102_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30 | USE_CLK_DUTY_30,
	.slave_func_fixed = true,
	.drv_timeout_n_offest = 8,
};

static struct sunxi_twi_hw_data sunxi_twi_v103_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30 | USE_CLK_DUTY_30,
	.slave_func_fixed = true,
	.dvfs_valid_chan_nums = 3,
	.drv_timeout_n_offest = 8,
};

static struct sunxi_twi_hw_data sunxi_twi_v104_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30 | USE_CLK_DUTY_30 | HAS_CLK_FREQ_1M,
	.slave_func_fixed = true,
	.drv_timeout_n_offest = 2,
};

static struct sunxi_twi_hw_data sunxi_twi_v105_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30,
	.slave_func_fixed = true,
	.drv_timeout_n_offest = 8,
};

static struct sunxi_twi_hw_data sunxi_twi_v106_data = {
	.clk_quirk_flag = HAS_CLK_DUTY_30,
	.slave_func_fixed = true,
	.drv_timeout_n_offest = 2,
};
...
/* set twi controller clock divided
 * clk_n: clock divider factor n
 * clk_m: clock divider factor m
 */
static void sunxi_twi_set_clock_div(struct sunxi_twi *twi, u8 clk_m, u8 clk_n)
{
	...
	case TWI_FREQ_200K:
		...
		break;
	case TWI_FREQ_400K:
		if (twi->clk_duty_30_enable && (twi->data->clk_quirk_flag & HAS_CLK_DUTY_30) &&
				(twi->data->clk_quirk_flag & USE_CLK_DUTY_30))
			/* Clock duty 30% */
			reg_val |= duty_30;
		else
			/* Clock duty 40% */
		...
		break;
	case TWI_FREQ_1M:
		if ((twi->data->clk_quirk_flag & HAS_CLK_DUTY_30) && (twi->data->clk_quirk_flag & HAS_CLK_FREQ_1M))
			/* Clock duty 30% */
			reg_val |= duty_30;
		else
			/* Do not support 1MHz*/
			sunxi_err(twi->dev, "The current platform does not support %d\n", twi->bus_freq);
		break;
	default:
...
static int sunxi_twi_set_frequency(struct sunxi_twi *twi)
{
	...
	if (twi->apb_clk) {
		if (twi->bus_freq >= TWI_FREQ_1M) {
			clk_set_rate(twi->apb_clk, APB_CLK_RATE_100M);
		} else {
			clk_set_rate(twi->apb_clk, APB_CLK_RATE_24M);
		}
		clk_in = clk_get_rate(twi->apb_clk);
	} else {
...
#if IS_ENABLED(CONFIG_I2C_SLAVE)
	if (twi->slave) {
		/* for slave mode, use the fixed clk_n divider for scl sampling, clk_m is not used. */
		switch (twi->bus_freq) {
		case TWI_FREQ_100K:
			if (clk_in > APB_CLK_RATE_24M)
				clk_n = 4;
			else
				clk_n = 2;
			break;
		case TWI_FREQ_400K:
			...
			break;
		default:
			sunxi_err(twi->dev, "unsupport slave freq %d\n", twi->bus_freq);
			return -EINVAL;
		}
		goto twi_set_clk;
	}
...
twi_set_clk:
	sunxi_twi_set_clock_div(twi, clk_m, clk_n);
...
static int sunxi_twi_engine_stop(struct sunxi_twi *twi)
{
	...
	err = wait_field_equ_maysleep((twi->base_addr + TWI_STAT), TWI_STAT_MASK,
				TWI_STAT_IDLE, 10, TWI_TIMEOUT_US);
	if (err) {
...
static void sunxi_twi_set_clk_count_mode(struct sunxi_twi *twi, bool mode)
{
...
/* clock cycles timeout for waiting ack response */
static void sunxi_twi_drv_set_timeout(struct sunxi_twi *twi, u16 timeout)
{
	u32 reg_val;
	unsigned int timeout_n_offest = twi->data->drv_timeout_n_offest;

	reg_val = readl(twi->base_addr + TWI_DRV_CTRL);
	if (timeout_n_offest == 2) {
		reg_val &= ~(TIMEOUT_N_2 << timeout_n_offest);
	} else {
		reg_val &= ~(TIMEOUT_N_8 << timeout_n_offest);
	}
	reg_val |= (timeout << timeout_n_offest);

	writel(reg_val, twi->base_addr + TWI_DRV_CTRL);
}

static void sunxi_twi_drv_stop(struct sunxi_twi *twi)
{
	u32 reg_val;

	reg_val = readl(twi->base_addr + TWI_DRV_BUS_CTRL);
	reg_val |= SCL_MOV;
	reg_val &= ~SDA_MOV;
	writel(reg_val, twi->base_addr + TWI_DRV_BUS_CTRL);
	...
	sunxi_twi_sda_control_disable(twi);
	sunxi_twi_scl_control_disable(twi);
	TWI_DBG(twi, "drv-mode: stop signal xfered");
}
...
static int sunxi_twi_drv_wait_complete(struct sunxi_twi *twi)
{
	...
	/* If it's in the atomic context at this time, print the call stack */
	sunxi_assert_non_atomic();
	...
	if (twi->status == SUNXI_TWI_XFER_STATUS_ERROR) {
		...
		sunxi_err(twi->dev, "drv-mode: result err %s\n", twi_status[twi->status]);
		return -EINVAL;
	}
...
static int sunxi_twi_engine_wait_complete(struct sunxi_twi *twi)
{
	...
	if (twi->status == SUNXI_TWI_XFER_STATUS_ERROR) {
		...
		sunxi_err(twi->dev, "engine-mode: result err %s\n", twi_status[twi->status]);
		ret = -EINVAL;
	}
...
{
	void __iomem *base_addr = twi->base_addr;
	unsigned int irq, eng_sta, drv_sta;
	int ret = 0;
	...
	if (!irq) {
		sunxi_err(twi->dev, "irq empty %#x\n", irq);
		twi->status = SUNXI_TWI_XFER_STATUS_ERROR;
		ret = -EINVAL;
		goto wake;
	}

	if (irq & (RF_UDF | RF_OVF)) {
		sunxi_err(twi->dev, "irq bus rxfifo underrun/overflow 0x%x\n", irq);
		twi->status = SUNXI_TWI_XFER_STATUS_ERROR;
		ret = -ENXIO;
		goto wake;
	}

	if (irq & (SF_UDF | SF_OVF)) {
		sunxi_err(twi->dev, "irq bus txfifo underrun/overflow 0x%x\n", irq);
		twi->status = SUNXI_TWI_XFER_STATUS_ERROR;
		ret = -ENXIO;
		goto wake;
	}

	if (irq & (SF_EMP | SF_FULL | RF_EMP | RF_FULL))
		sunxi_debug(twi->dev, "irq bus fifo status 0x%x\n", irq);

	if (irq & TRAN_ERR_PD) { /* current read/write packages trasnfer err */
		/* drv mode cannot feedback the real state of statemachine,
		 * So use eng mode to confirm the status.
		 */
		drv_sta = sunxi_twi_get_xfer_sta(twi);
		eng_sta = readl(twi->base_addr + TWI_STAT) & TWI_STAT_MASK;
		...
		sunxi_err(twi->dev, "drv mode: TWI BUS error state is drv:0x%x eng:0x%x\n", drv_sta, eng_sta);
		twi->status = SUNXI_TWI_XFER_STATUS_ERROR;
		ret = -eng_sta;
		goto wake;
	}
	...
	if (irq & TRAN_COM_PD) { /* read or write packages is complete */
		/*
	...
	if (irq & RX_REQ_PD) { /* current read operation is end */
		if (sunxi_twi_drv_recv_msg(twi, twi->msg))
			twi->status = SUNXI_TWI_XFER_STATUS_ERROR;
		else
			twi->status = SUNXI_TWI_XFER_STATUS_COMPLETE;
...
static irqreturn_t sunxi_twi_handler_thread(int this_irq, void *dev_id)
{
	struct sunxi_twi *twi = (struct sunxi_twi *)dev_id;

	mutex_lock(&twi->thread_lock);
...
static irqreturn_t sunxi_twi_handler(int this_irq, void *dev_id)
{
	...
	if (twi->slave) {
		if (ret == TWI_ENGINE_IRQ) {
			...
			sunxi_debug(twi->dev, "slave-mode: wrong irq, check irq number!!\n");
			return IRQ_NONE;
		}
	} else
	...
	{
		if (ret == TWI_ENGINE_IRQ) {
			return sunxi_twi_engine_core_process(twi);
			...
			if (twi->twi_dvfs_enable) {
				sunxi_twi_dvfs_core_process(twi);
				return IRQ_HANDLED;
			} else
#endif
			{
				sunxi_debug(twi->dev, "master-mode: wrong irq, check irq number!!\n");
				return IRQ_NONE;
			}
		}
	}
...
static int sunxi_twi_drv_tx_one_msg(struct sunxi_twi *twi, struct i2c_msg *msg)
{
	...
	sunxi_twi_drv_enable_irq(twi->base_addr, TRAN_ERR_INT_EN | TRAN_COM_INT_EN | SUNXI_TWI_SEND_INT_ERR);
...
sunxi_twi_drv_rx_msgs(struct sunxi_twi *twi, struct i2c_msg *msgs, int num)
{
	...
	if (rmsg->len > MAX_FIFO)
		sunxi_twi_drv_enable_irq(twi->base_addr, TRAN_ERR_INT_EN | TRAN_COM_INT_EN | SUNXI_TWI_RECV_INT_ERR);
	else
		sunxi_twi_drv_enable_irq(twi->base_addr, TRAN_ERR_INT_EN | TRAN_COM_INT_EN | RX_REQ_INT_EN | SUNXI_TWI_RECV_INT_ERR);
...
sunxi_twi_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	if (twi->status == SUNXI_TWI_XFER_STATUS_SHUTDOWN || twi->status == SUNXI_TWI_XFER_STATUS_RUNNING) {
		spin_unlock_irqrestore(&twi->lock, flags);
		sunxi_err(twi->dev, "xfer busy with status: %s\n", twi_status[twi->status]);
		return -EBUSY;
	} else {
...
{
	twi->apb_clk = devm_clk_get_optional(twi->dev, "apb");
	if (IS_ERR(twi->apb_clk)) {
		sunxi_err(twi->dev, "request apb clock failed\n");
		return -EINVAL;
	}
	...
	if (IS_ERR(twi->bus_clk)) {
		...
		if (IS_ERR_OR_NULL(twi->bus_clk)) {
			sunxi_err(twi->dev, "request clock failed\n");
			return -EINVAL;
		}
	}

	twi->reset = devm_reset_control_get_optional(twi->dev, NULL);
	if (IS_ERR(twi->reset)) {
...
#if IS_ENABLED(CONFIG_AW_TWI_DVFS)
	twi->twi_dvfs_enable = false;
	if (twi->twi_drv_used) {
		twi->twi_dvfs_enable = of_property_read_bool(np, "twi_dvfs_enable");
		if (twi->twi_dvfs_enable && !twi->no_suspend) {
			sunxi_warn(twi->dev, "dvfs should work in drv and no_suspend mode\n");
			twi->twi_dvfs_enable = false;
		}

		if (twi->twi_dvfs_enable) {
			twi->dvfs_devices = devm_kzalloc(twi->dev, sizeof(*twi->dvfs_devices), GFP_KERNEL);
			if (IS_ERR_OR_NULL(twi->dvfs_devices)) {
				err = -ENOMEM;  /* Do not print prompts after kzalloc errors */
				goto err0;
			}
			if (twi->data->dvfs_valid_chan_nums > DVFS_CHAN_MAX) {
				sunxi_err(twi->dev, "dvfs_valid_chan_nums exceed the max channel number %d\n", DVFS_CHAN_MAX);
				err = -EINVAL;
				goto err0;
			}
			twi->dvfs_devices->dvfs_channel = devm_kzalloc(twi->dev, twi->data->dvfs_valid_chan_nums *
								sizeof(*twi->dvfs_devices->dvfs_channel), GFP_KERNEL);
			if (IS_ERR_OR_NULL(twi->dvfs_devices->dvfs_channel)) {
				err = -ENOMEM;
				goto err0;
			}
		}
...
#endif
	twi->clk_duty_30_enable = true;
	if (of_property_read_bool(np, "clk_duty_30_disable")) {
		sunxi_debug(twi->dev, "clk duty 30 disable");
		twi->clk_duty_30_enable = false;
	}

	if (!twi->twi_drv_used)
		twi->irq_flag |= IRQF_ONESHOT;
...
static int sunxi_twi_clk_init(struct sunxi_twi *twi)
{
	...
	if (twi->apb_clk) {
		if (clk_prepare_enable(twi->apb_clk)) {
			sunxi_err(twi->dev, "enable apb clock failed!\n");
			err = -EINVAL;
			goto err0;
		}
	}
...
static int sunxi_twi_bus_set_clk(struct sunxi_twi *twi)
{
	int ret;

	/* twi ctroller module clock is controllerd by self */
	ret = sunxi_twi_set_frequency(twi);
	if (ret) {
		sunxi_err(twi->dev, "set frequency failed!\n");
		goto out;
	}

	/* set 1 use scl level sampling from input */
	sunxi_twi_set_clk_count_mode(twi, 1);

	/* set maximum timeout cycle. For drv only, eng not support */
	if (twi->twi_drv_used) {
		if (twi->data->drv_timeout_n_offest == 2) {
			sunxi_twi_drv_set_timeout(twi, TIMEOUT_N_2);
		} else {
			sunxi_twi_drv_set_timeout(twi, TIMEOUT_N_8);
		}
	}
...
static int sunxi_twi_hw_init(struct sunxi_twi *twi)
{
	...
	err = sunxi_twi_bus_set_clk(twi);
	if (err) {
		sunxi_err(twi->dev, "set bus clk failed!\n");
		goto err3;
	}
...
static int sunxi_twi_rpmsg_callback(void *dev, void *data, int len)
{
	...
	if (!data || !strncmp(data, "return", len)) {
		err = devm_request_threaded_irq(twi->dev, twi->irq, sunxi_twi_handler,
				sunxi_twi_handler_thread, twi->irq_flag, dev_name(twi->dev), twi);
		if (err) {
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case TWI_FREQ_1M:
		...
		break;
	default:
		sunxi_err(twi->dev, "Invalid input %u. Should be 100000/200000/400000/1000000.\n", freq);
		break;
	}
...
static int sunxi_twi_probe(struct platform_device *pdev)
{
	...
	err = sunxi_pdev_request_peri_rsc(pdev, "sunxi_twi_drv", &twi->amp_rsc);
	if (err) {
		sunxi_err(twi->dev, "request AMP system peri resource failed, ret: %d\n", err);
		return -EINVAL;
	}
#endif
...
#ifdef CONFIG_AW_AMP_SYS_RSC_MANAGER
	ret = sunxi_amp_rsc_free(twi->amp_rsc);
	if (ret) {
		sunxi_err(twi->dev, "release AMP system resource for failed, ret: %d\n", ret);
		return ret;
	}
#endif
...
static int sunxi_twi_suspend_late(struct device *dev)
{
	...
	if (twi->no_suspend) {
		sunxi_info(twi->dev, "suspend late no_suspend\n");
		return 0;
	}
...
static int sunxi_twi_resume_early(struct device *dev)
{
	 ...
	 */
	if (twi->no_suspend) {
		/* Test suspend bit to confirm adapter mark is symmetrical.
		 * For some case suspend/resume_noirq will not run.
		 * Such as a PM pending wakeup source is actived after suspend_late().
		 */
		if (test_bit(I2C_ALF_IS_SUSPENDED, &twi->adap.locked_flags)) {
			ret = sunxi_twi_hw_init(twi);
			spin_lock_irqsave(&twi->lock, flags);
			twi->status = SUNXI_TWI_XFER_STATUS_IDLE;
			spin_unlock_irqrestore(&twi->lock, flags);
			i2c_mark_adapter_resumed(&twi->adap);
			sunxi_info(twi->dev, "resume early no_suspend\n");
		}
		return 0;
	}
...
static const struct of_device_id sunxi_twi_match[] = {
	...
	/* For sun60iw1/sun55iw3 */
	{ .compatible = "allwinner,sunxi-twi-v101", .data = &sunxi_twi_v101_data },
	/* sun55iw6/sun65iw1/sun251iw1 */
	{ .compatible = "allwinner,sunxi-twi-v102", .data = &sunxi_twi_v102_data },
	/* sun65iw1 */
	{ .compatible = "allwinner,sunxi-twi-v103", .data = &sunxi_twi_v103_data },
	/* sun8iw22 */
	{ .compatible = "allwinner,sunxi-twi-v104", .data = &sunxi_twi_v104_data },
	/* sun50iw15 */
	{ .compatible = "allwinner,sunxi-twi-v105", .data = &sunxi_twi_v105_data },
	/* sun55iw7 */
	{ .compatible = "allwinner,sunxi-twi-v106", .data = &sunxi_twi_v106_data },
	{},
};
...
postcore_initcall(sunxi_twi_adap_init);
