...
static int pvr_device_clk_init(struct pvr_device *pvr_dev)
{
	...
	clk_parent = devm_clk_get(drm_dev->dev, "clk_parent");
	if (IS_ERR(clk_parent))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_parent),
				     "failed to get parent clock\n");

	clk = devm_clk_get(drm_dev->dev, "clk");
	if (IS_ERR(clk))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk),
				     "failed to get clock\n");

	clk_bus = devm_clk_get(drm_dev->dev, "clk_bus");
	if (IS_ERR(clk_bus))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_bus),
				     "failed to get clock bus\n");
	pvr_dev->clk_parent = clk_parent;
	pvr_dev->clk = clk;
	pvr_dev->clk_bus = clk_bus;

#if defined(CONFIG_OF)
	struct clk *clk_200, *clk_300, *clk_400, *clk_600, *clk_800;
	clk_200 = devm_clk_get(drm_dev->dev, "clk_200");
	if (IS_ERR(clk_200))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_200),
				     "failed to get clock 200\n");

	clk_300 = devm_clk_get(drm_dev->dev, "clk_300");
	if (IS_ERR(clk_300))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_300),
				     "failed to get clock 300\n");

	clk_400 = devm_clk_get(drm_dev->dev, "clk_400");
	if (IS_ERR(clk_400))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_400),
				     "failed to get clock 400\n");

	clk_600 = devm_clk_get(drm_dev->dev, "clk_600");
	if (IS_ERR(clk_600))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_600),
				     "failed to get clock 600\n");

	clk_800 = devm_clk_get(drm_dev->dev, "clk_800");
	if (IS_ERR(clk_800))
		return dev_err_probe(drm_dev->dev, PTR_ERR(clk_800),
				     "failed to get clock 800\n");

	pvr_dev->clk_200 = clk_200;
	pvr_dev->clk_400 = clk_400;
	pvr_dev->clk_300 = clk_300;
	pvr_dev->clk_600 = clk_600;
	pvr_dev->clk_800 = clk_800;
#endif
#else
	struct clk *core_clk;
	struct clk *sys_clk;
...
pvr_device_irq_init(struct pvr_device *pvr_dev)
{
	...
	pvr_dev->dvfs_irq = platform_get_irq_byname(pdev, "IRQGPUDVFS");
	if (sunxi_data->dvfs_irq < 0) {
		dev_err(dev, "failed to get dvfs irq number from device tree");
		return pvr_dev->dvfs_irq;
	}
#endif
	...
	return request_threaded_irq(pvr_dev->irq, pvr_device_irq_handler,
				    ...
				    IRQF_SHARED | IRQF_ONESHOT, "IRQGPU", pvr_dev);
}
...
pvr_request_firmware(struct pvr_device *pvr_dev)
{
	...
	if (err == -ENOENT) {
		if (!fw_once) {
			kfree(filename);
			drm_warn(drm_dev, "firmware not available yet, deferring\n");
			fw_once = true;
		}
		return -EPROBE_DEFER;
	}
