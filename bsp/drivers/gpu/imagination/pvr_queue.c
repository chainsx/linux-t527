...
#else
	if (frag_job->base.s_fence && frag_job->base.s_fence->parent) {
		f = frag_job->base.s_fence->parent;
		if (!dma_fence_is_signaled(f) &&
				f != &job->base.s_fence->scheduled) {
			return dma_fence_get(f);
		}
	}
	return frag_job->base.sched->ops->dependency(&frag_job->base, &queue->entity);
#endif
}
...
#else
	drm_err(from_pvr_device(pvr_dev), "Job timeout\n");
#endif
...
#else
	.dependency = pvr_queue_prepare_job,
#endif
...
#else
	drm_sched_entity_push_job(&job->base, job->base.entity);
#endif
}
				   ...
				   void *fw_ctx_map)
{
	...
	err = drm_sched_init(&queue->scheduler,
			     &pvr_queue_sched_ops,
			     64 * 1024, 1, msecs_to_jiffies(500),
#if (LINUX_VERSION_CODE > KERNEL_VERSION(6, 0, 0))
